...
async function lsEntries(
  ...
  if (listDir) {
    const s = await langfuseStat(accessor, path, indexCache ?? undefined)
    return [s]
  }
  let entries: string[]
  try {
    entries = await langfuseReaddir(accessor, path, indexCache ?? undefined)
  } catch (err) {
  ...
  for (const entry of entries) {
    try {
      ...
      const s = await langfuseStat(accessor, eSpec, indexCache ?? undefined)
...
async function lsCommand(
  ...
  workingPaths = await resolveLangfuseGlob(accessor, workingPaths, opts.index ?? undefined)
