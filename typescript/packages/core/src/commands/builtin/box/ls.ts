...
async function lsEntries(
  ...
  if (listDir) {
    const s = await boxStat(accessor, path, indexCache ?? undefined)
    return [s]
  }
  let entries: string[]
  try {
    entries = await boxReaddir(accessor, path, indexCache ?? undefined)
  } catch (err) {
  ...
  for (const entry of entries) {
    try {
      ...
      const s = await boxStat(accessor, eSpec, indexCache ?? undefined)
