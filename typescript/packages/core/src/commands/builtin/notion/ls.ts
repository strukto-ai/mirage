...
async function lsEntries(
  ...
  if (listDir) {
    const s = await notionStat(accessor, path, indexCache ?? undefined)
    return [s]
  }
  let entries: string[]
  try {
    entries = await notionReaddir(accessor, path, indexCache ?? undefined)
  } catch (err) {
  ...
  for (const entry of entries) {
    try {
      ...
      const s = await notionStat(accessor, eSpec, indexCache ?? undefined)
...
async function lsCommand(
  ...
  workingPaths = await resolveNotionGlob(accessor, workingPaths, opts.index ?? undefined)
...
export const NOTION_LS = command({
  name: 'ls',
