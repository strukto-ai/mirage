...
async function lsEntries(
  ...
  if (listDir) {
    const s = await trelloStat(accessor, path, indexCache ?? undefined)
    return [s]
  }
  let entries: string[]
  try {
    entries = await trelloReaddir(accessor, path, indexCache ?? undefined)
  } catch (err) {
  ...
  for (const entry of entries) {
    try {
      ...
      const s = await trelloStat(accessor, eSpec, indexCache ?? undefined)
...
async function lsCommand(
  ...
  workingPaths = await resolveTrelloGlob(accessor, workingPaths, opts.index ?? undefined)
...
export const TRELLO_LS = command({
  name: 'ls',
