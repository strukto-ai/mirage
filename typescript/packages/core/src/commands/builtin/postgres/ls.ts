...
async function lsEntries(
  ...
  if (listDir) {
    const s = await postgresStat(accessor, path, indexCache ?? undefined)
    return [s]
  }
  let entries: string[]
  try {
  ...
  for (const entry of entries) {
    try {
      ...
      const s = await postgresStat(accessor, eSpec, indexCache ?? undefined)
...
export const POSTGRES_LS = command({
  name: 'ls',
