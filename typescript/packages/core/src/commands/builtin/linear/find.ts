...
// Mirrors the Python linear find _walk: the search root is always included,
...
async function walk(
  ...
  let info
  try {
    info = await linearStat(accessor, path, index)
  } catch {
    return results
  }
...
async function findCommand(
  ...
  for (const entryPath of [...allPaths].sort()) {
    ...
    const info = await linearStat(accessor, entrySpec, opts.index ?? undefined)
