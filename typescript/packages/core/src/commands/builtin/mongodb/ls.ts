...
async function lsEntries(
  ...
  if (listDir) {
    const s = await mongoStat(accessor, path, indexCache ?? undefined)
    return [s]
  }
  let entries: string[]
  try {
    entries = await mongoReaddir(accessor, path, indexCache ?? undefined)
  } catch (err) {
  ...
  for (const entry of entries) {
    try {
      ...
      const s = await mongoStat(accessor, eSpec, indexCache ?? undefined)
...
export const MONGODB_LS = command({
  name: 'ls',
