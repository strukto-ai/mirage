...
import { sendMessage } from '../../../core/discord/post.ts'
...
async function discordSendMessageCommand(
  ...
  const ref = typeof messageId === 'string' && messageId !== '' ? messageId : undefined
  const result = await sendMessage(accessor, channelId, text, ref)
  return [ENC.encode(JSON.stringify(result)), new IOResult()]
}

export const DISCORD_SEND_MESSAGE = command({
