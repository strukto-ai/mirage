...
async function lsEntries(
  ...
  if (listDir) {
    const s = await dropboxStat(accessor, path, indexCache ?? undefined)
    return [s]
  }
  let entries: string[]
  try {
    entries = await dropboxReaddir(accessor, path, indexCache ?? undefined)
  } catch (err) {
  ...
  for (const entry of entries) {
    try {
      ...
      const s = await dropboxStat(accessor, eSpec, indexCache ?? undefined)
...
export const DROPBOX_LS = command({
  name: 'ls',
