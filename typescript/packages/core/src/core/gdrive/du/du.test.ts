...
describe('gdrive core du', () => {
  it('sums file sizes across a nested tree', async () => {
    seedTree()
    expect(await size(accessor, spec('/'))).toBe(4 + 2048 + 16)
  })

  it('a file path resolves from its own stat', async () => {
    seedTree()
    expect(await size(accessor, spec('/a.txt'))).toBe(4)
    expect(await entries(accessor, spec('/a.txt'))).toEqual([[], 4])
  })

  it('entries returns per-file entries plus the total', async () => {
    seedTree()
    const [found, total] = await entries(accessor, spec('/sub'))
  ...
  it('a missing root raises ENOENT', async () => {
    await expect(size(accessor, spec('/missing'))).rejects.toMatchObject({ code: 'ENOENT' })
  })
})
