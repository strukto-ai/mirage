...
describe('gsheets detectScope', () => {
  ...
  it('splits a file into label and id', () => {
    const match = detectScope(ps('/owned/2024-01-05_Notes__abc12.gsheet.json'))
  ...
  it('matches a shared file too', () => {
    const match = detectScope(ps('/shared/Plan__xyz.gsheet.json'))
    expect(match.kind).toBe('file')
  ...
  it('answers invalid for unrecognized shapes', () => {
    ...
    expect(detectScope(ps('/bogus/File__id.gsheet.json')).kind).toBe('invalid')
    expect(detectScope(ps('/owned/plain.gsheet.json')).kind).toBe('invalid')
    expect(detectScope(ps('/owned/File__id.wrong.json')).kind).toBe('invalid')
    expect(detectScope(ps('/owned/File__id.gsheet.json/deep')).kind).toBe('invalid')
  })
})
