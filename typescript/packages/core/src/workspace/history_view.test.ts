...
describe('history view + builtin', () => {
  it('history lists only the calling session', async () => {
    ...
    await ws.shell('pwd')
    await ws.shell('ls /', { sessionId: 's2' })
    const mine = out(await ws.shell('history'))
    const other = out(await ws.shell('history', { sessionId: 's2' }))
  ...
  it('cat /.bash_history shows all sessions in histfile format', async () => {
    ...
    ws.createSession('s2')
    await ws.shell('ls /data')
    await ws.shell('pwd', { sessionId: 's2' })
  ...
  it('grep and tail work over /.bash_history', async () => {
    ...
    await ws.shell('ls /data')
    await ws.shell('pwd')
    const grep = await ws.shell('grep pwd /.bash_history')
    expect(grep.exitCode).toBe(0)
    expect(out(grep)).toContain('pwd')
    expect(out(grep)).not.toContain('ls /data')
    const tail = await ws.shell('tail -n 2 /.bash_history')
  ...
  it('ls / hides the dotfile, ls -a shows it', async () => {
    const ws = makeWs()
    expect(out(await ws.shell('ls /'))).not.toContain('.bash_history')
    expect(out(await ws.shell('ls -a /'))).toContain('.bash_history')
    await ws.close()
  })
  ...
  it('history -c clears only the calling session; file unchanged for others', async () => {
    ...
    await ws.shell('ls /data')
    await ws.shell('pwd', { sessionId: 's2' })
    const clear = await ws.shell('history -c')
    expect(clear.exitCode).toBe(0)
    expect(out(await ws.shell('history'))).not.toContain('ls /data')
    expect(out(await ws.shell('history', { sessionId: 's2' }))).toContain('pwd')
    expect(out(await ws.shell('cat /.bash_history'))).toContain('ls /data')
    await ws.close()
  })

  it('appending to /.bash_history is rejected', async () => {
    const ws = makeWs()
    expect((await ws.shell('echo hacked >> /.bash_history')).exitCode).not.toBe(0)
    await ws.close()
  })

  it('/.sessions no longer resolves', async () => {
    const ws = makeWs()
    expect((await ws.shell('ls /.sessions')).exitCode).not.toBe(0)
    await ws.close()
  })
...
describe('history builtin flags', () => {
  it('-s appends without executing', async () => {
    const ws = makeWs()
    await ws.shell('pwd')
    const res = await ws.shell('history -s rm -rf /data')
    expect(res.exitCode).toBe(0)
    expect(out(await ws.shell('history'))).toContain('rm -rf /data')
    expect((await ws.shell('ls /data/x')).exitCode).not.toBe(0)
    await ws.close()
  })
  ...
  it('-d deletes and renumbers', async () => {
    const ws = makeWs()
    await ws.shell('pwd')
    await ws.shell('echo keep')
    expect((await ws.shell('history -d 1')).exitCode).toBe(0)
    expect(out(await ws.shell('history')).startsWith('1  echo keep')).toBe(true)
    await ws.close()
  })
  ...
  it('-d with an attached offset deletes', async () => {
    ...
    await ws.shell('pwd')
    await ws.shell('echo keep')
    expect((await ws.shell('history -d1')).exitCode).toBe(0)
    expect(out(await ws.shell('history')).startsWith('1  echo keep')).toBe(true)
    await ws.close()
  })
  ...
  it('-d out of range exits 1', async () => {
    const ws = makeWs()
    await ws.shell('pwd')
    const res = await ws.shell('history -d 99')
  ...
  it('-d non-numeric exits 1', async () => {
    const ws = makeWs()
    const res = await ws.shell('history -d abc')
    expect(res.exitCode).toBe(1)
  ...
  it('-d with trailing garbage is rejected (1abc, not parsed as 1)', async () => {
    const ws = makeWs()
    await ws.shell('pwd')
    const res = await ws.shell('history -d 1abc')
    expect(res.exitCode).toBe(1)
  ...
  it('a count with trailing garbage is rejected (3abc, not parsed as 3)', async () => {
    const ws = makeWs()
    await ws.shell('pwd')
    const res = await ws.shell('history 3abc')
    expect(res.exitCode).toBe(1)
  ...
  it('-d negative offset deletes the last entry', async () => {
    const ws = makeWs()
    await ws.shell('echo first')
    await ws.shell('echo last')
    expect((await ws.shell('history -d -1')).exitCode).toBe(0)
    const listing = out(await ws.shell('history'))
  ...
  it('-d requires an argument (usage, exit 2)', async () => {
    const ws = makeWs()
    const res = await ws.shell('history -d')
  ...
  it('invalid option is a usage error (exit 2)', async () => {
    const ws = makeWs()
    const res = await ws.shell('history -z')
    expect(res.exitCode).toBe(2)
  ...
  it('-p prints args without storing them as a command', async () => {
    const ws = makeWs()
    const res = await ws.shell('history -p hello world')
    expect(res.exitCode).toBe(0)
    expect(out(res)).toBe('hello\nworld\n')
    expect(out(await ws.shell('history'))).toContain('1  history -p hello world')
    await ws.close()
  })

  it('-ps suppresses the print', async () => {
    const ws = makeWs()
    const res = await ws.shell('history -ps echo hi')
    expect(res.exitCode).toBe(0)
    expect(out(res)).toBe('')
    expect(out(await ws.shell('history'))).toContain('echo hi')
    await ws.close()
  })
  ...
  it('history 0 lists nothing', async () => {
    ...
    const res = await ws.shell('history 0')
    expect(res.exitCode).toBe(0)
    expect(out(res)).toBe('')
    await ws.close()
  })
  ...
  it('-a/-r/-w/-n are no-ops', async () => {
    ...
    for (const flag of ['-a', '-r', '-w', '-n']) {
      const res = await ws.shell(`history ${flag}`)
      expect(res.exitCode).toBe(0)
      expect(out(res)).toBe('')
    }
    await ws.close()
  ...
  it('-cw clears and the listing is empty after', async () => {
    const ws = makeWs()
    await ws.shell('pwd')
    expect((await ws.shell('history -cw')).exitCode).toBe(0)
    expect(out(await ws.shell('history 1'))).not.toContain('pwd')
    await ws.close()
  })

  it('find resolves the single view entry via the generic', async () => {
    const ws = makeWs()
    expect(out(await ws.shell('find /.bash_history'))).toBe('/.bash_history\n')
    expect(out(await ws.shell("find /.bash_history -name '*.bash*'"))).toBe('/.bash_history\n')
    expect(out(await ws.shell('find /.bash_history -type d'))).toBe('')
    await ws.close()
  })
})
...
describe('history recording boundaries (GNU line-reader semantics)', () => {
  it('command substitution records only the outer line', async () => {
    ...
    await ws.shell("wc -l $(find /data -name '*.txt')")
  ...
  it('substitution keeps inner ops in the audit log', async () => {
    const ws = makeWs()
    await ws.shell('echo hi > /data/f.txt')
    await ws.shell('echo $(cat /data/f.txt)')
  ...
  it('eval / xargs / source each record a single entry', async () => {
    const ws = makeWs()
    await ws.shell('echo hi > /data/f.txt')
    await ws.shell('eval cat /data/f.txt')
    await ws.shell('echo /data/f.txt | xargs cat')
    await ws.shell("echo 'cat /data/f.txt' > /data/s.sh")
    await ws.shell('source /data/s.sh')
    expect(await commands(ws)).toEqual([
      'echo hi > /data/f.txt',
      'eval cat /data/f.txt',
  ...
  it('a blank line is not recorded but whitespace and comments are', async () => {
    ...
    await ws.shell('\n')
    await ws.shell('   ')
    await ws.shell('# comment')
    expect(await commands(ws)).toEqual(['echo one', '   ', '# comment'])
    const file = out(await ws.shell('cat /.bash_history'))
  ...
  it('an unrecorded execute skips history but keeps its ops', async () => {
    const ws = makeWs()
    await ws.shell('echo hi > /data/f.txt')
    await ws.shell('cat /data/f.txt', { record: false })
...
describe('history snapshot rewind', () => {
  it('snapshot + load preserves clear tombstones', async () => {
    ...
    expect(mine).toContain('pwd')
    expect(out(await dst.shell('cat /.bash_history'))).toContain('ls /data')
    await ws.close()
    await dst.close()
  })
  ...
  it('in-place restore rewinds the recorder to the snapshot timeline', async () => {
    const src = makeWs()
    await src.shell('echo from-snapshot')
    const state = await toStateDict(src)
    const dst = makeWs()
    await dst.shell('echo pre-restore')
