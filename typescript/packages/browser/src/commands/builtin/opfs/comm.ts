...
async function commCommand(
  ...
  const data1 = DEC.decode(await materialize(opfsStream(accessor.rootHandle, p1)))
  const data2 = DEC.decode(await materialize(opfsStream(accessor.rootHandle, p2)))
...
export const OPFS_COMM = command({
  name: 'comm',
