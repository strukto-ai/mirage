...
export class PrefetchCache {
  ...
  /** Whether a fill for `path` is still running. */
  isFilling(path: string): boolean {
   ...
   *
   * `fresh` is asked immediately before the install, and a false answer
   * returns the bytes to the caller without caching them. Only the core
   * knows whether the file changed while the fill was out, and the
   * question has to be asked here rather than inside `fill`: between a
   * fill returning and this installing what it returned there is an
   * await, which is one more chance for a mutation to land.
   */
  claim(
    path: string,
    fill: () => Promise<Uint8Array | null>,
    fresh?: () => boolean,
    ...
    const promise = (async (): Promise<Uint8Array | null> => {
      try {
        ...
        if (data !== null && (fresh === undefined || fresh())) this.put(path, data)
        return data
      } finally {
