...
export function tencentToS3Config(config: TencentConfig): S3Config {
  return {
    ...
    endpoint: resolvedTencentEndpoint(config),
...
export function redactTencentConfig(config: TencentConfig): TencentConfigRedacted {
  return redactConfigWithSchema(TencentConfigSchema, {
    ...config,
    endpoint: resolvedTencentEndpoint(config),
  }) as unknown as TencentConfigRedacted
}

export function normalizeTencentConfig(input: Record<string, unknown>): TencentConfig {
