...
export interface DigitalOceanConfig extends S3AliasConfig {
  region: string
}

export type DigitalOceanConfigRedacted = S3AliasConfigRedacted

const alias = makeRegionAlias<DigitalOceanConfig, DigitalOceanConfigRedacted>(
  (config) => `https://${config.region}.digitaloceanspaces.com`,
)
