...
export interface SSHResourceLike extends Resource {
...
async function resolveSizes(
  ...
  for (const p of paths) {
    ...
    if (size === null) {
      try {
        const fileStat = await sshStat(accessor, p)
        size = fileStat.size
      } catch {
...
export async function fileReadProvision(
  ...
  const { resolved, missing } = await resolveSizes(accessor as SSHAccessor, paths, undefined)
...
export async function headTailProvision(
  ...
  if (cRaw !== null) {
    const cBytes = Number.parseInt(cRaw, 10)
    const total = resolved.reduce((sum, [, size]) => sum + Math.min(cBytes, size), 0)
    return new ProvisionResult({
      networkReadLow: total,
      networkReadHigh: total,
      readOps: resolved.length,
