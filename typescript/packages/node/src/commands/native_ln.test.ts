...
describe.each(NATIVE_BACKENDS)('native ln (%s backend)', (kind) => {
  it('ln -s -v prints -> arrow', async () => {
    const env = makeEnv(kind)
    try {
      env.createFile('a.txt', ENC.encode('hello'))
      const result = await env.mirage('ln -s -v /data/a.txt /data/link.txt')
  ...
  it('ln -s -f overwrites existing link', async () => {
    const env = makeEnv(kind)
    try {
      env.createFile('a.txt', ENC.encode('hello'))
  ...
  it('ln -s -n accepts flag and creates link', async () => {
    const env = makeEnv(kind)
    try {
      env.createFile('a.txt', ENC.encode('hello'))
