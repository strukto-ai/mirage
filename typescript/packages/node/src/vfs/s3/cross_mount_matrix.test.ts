...
function buildMount(kind: BackendKind, tmpRoot: string, idx: number): MountState {
  if (kind === 'ram') {
    return { kind, vfs: new RAMVFS(), diskRoot: null, s3Bucket: null }
  }
  if (kind === 'disk') {
    const root = path.join(tmpRoot, `disk${idx.toString()}`)
    mkdirSync(root, { recursive: true })
    return { kind, vfs: new DiskVFS({ root }), diskRoot: root, s3Bucket: null }
  }
  const bucket = `test-bucket-${idx.toString()}`
  return { kind, vfs: new S3VFS(s3Config(bucket)), diskRoot: null, s3Bucket: bucket }
}
