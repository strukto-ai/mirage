...
- **One `backend` field per mount: `vfs | fuse | fskit`** (`MountBackend` in
  ...
  live mount and then fails with ENOENT on the first read. The advisory
  `integ-fskit-macos` job (`integ/fskit.py`) is where this runs for real;
  it is the only coverage of an actual FSKit mount.
