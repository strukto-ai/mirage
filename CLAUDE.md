...
- **One `backend` field per mount: `vfs | fuse | fskit`** (`MountBackend` in
  ...
  **FSKit mounts are read-mostly.** Reads, in-place writes and `unlink` work;
  ...
  runs on a Mac with macFUSE 5.x: the `integ-fskit-macos` job is advisory
  because a hosted runner installs and enables everything but the mount
  request never reaches macFUSE.
