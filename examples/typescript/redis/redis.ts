...
async function main(): Promise<void> {
  ...
  await runLabeled(
    ws,
    "head -n 1 /data/reports/q1.csv",
    "head -n 1 /data/reports/q1.csv",
  );
  await runLabeled(
    ws,
    "tail -n 1 /data/reports/q1.csv",
    "tail -n 1 /data/reports/q1.csv",
  );
  ...
  await runLabeled(
    ws,
    "METADATA (chmod / chown / touch, native)",
    "ls -l /data/hello.txt",
  );
  await runLabeled(
    ws,
    "jq .name /data/user.json",
    'jq ".name" /data/user.json',
  );
  ...
  await runLabeled(
    ws,
    "sort /data/reports/q1.csv",
    "sort /data/reports/q1.csv",
  );
  await runLabeled(
    ws,
    "tr a-z A-Z < /data/hello.txt",
    "cat /data/hello.txt | tr a-z A-Z",
  );
  ...
  await runLabeled(
    ws,
    "awk",
    `cat /data/reports/q1.csv | awk -F, '{print $1}'`,
  );
  ...
  console.log(
    "  every executed command is recorded in GNU bash histfile format",
  );
  console.log("");
  ...
  if (!(wsProv instanceof ProvisionResult))
    throw new Error("expected ProvisionResult");
  ...
  const readCost = await fileReadProvision(
    vfs.accessor,
    paths,
    [],
    provisionOpts("cat"),
  ...
  const headCost = await headTailProvision(
    vfs.accessor,
    paths,
    [],
    provisionOpts("head -n 1"),
  );
  console.log(
    `  headTailProvision(...) — Redis fetches full value regardless of -n:`,
  );
  console.log(`    networkRead    = ${headCost.networkRead} bytes`);

  const metaCost = metadataProvision(
    vfs.accessor,
    paths,
    [],
    provisionOpts("stat"),
  ...
  console.log(
    `  metadataProvision(...) — stat/ls/find cost zero network bytes:`,
  );
  ...
  console.log("");
  console.log(
    "  Redis is a remote store — snapshot dumps the full key+value set.",
  );
  console.log(
    "  `copy()` shares the same Redis instance (no independent state).",
  );
  console.log("");
