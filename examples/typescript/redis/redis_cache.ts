...
async function main(): Promise<void> {
  ...
  console.log(
    `  matches same fp: ${String(await cache.isFresh("/data/hello.txt", fp))}`,
  );
  console.log(
    `  matches other fp: ${String(await cache.isFresh("/data/hello.txt", "abc"))}`,
  );

  console.log("\n--- add (no-op if present, true if inserted) ---");
  console.log(
    `  add existing: ${String(await cache.add("/data/hello.txt", payload))}`,
  );
  console.log(
    `  add new: ${String(await cache.add("/data/new.bin", new Uint8Array([1, 2, 3])))}`,
  );
  ...
  for (let i = 0; i < keys.length; i++) {
    const r = results[i];
    console.log(
      `  ${keys[i]}: ${r === null ? "MISS" : `${String(r.byteLength)} bytes`}`,
    );
  }
  ...
  console.log(
    "  another RedisFileCacheStore with same keyPrefix sees the same data:",
  );
