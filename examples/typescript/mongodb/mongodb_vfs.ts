...
async function main(): Promise<void> {
  ...
  try {
    ...
    for (const db of await ws.vfs.readdir('/mongodb')) console.log(`  ${db}`)

    console.log(`\n--- listdir() /mongodb/${DB} (entities) ---`)
    for (const e of await ws.vfs.readdir(`/mongodb/${DB}`)) console.log(`  ${e}`)

    console.log(`\n--- listdir() /mongodb/${DB}/collections ---`)
    for (const c of await ws.vfs.readdir(`/mongodb/${DB}/collections`)) console.log(`  ${c}`)

    console.log(`\n--- listdir() /mongodb/${DB}/views ---`)
    for (const v of await ws.vfs.readdir(`/mongodb/${DB}/views`)) console.log(`  ${v}`)
