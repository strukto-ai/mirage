...
function configFromEnv(): TencentConfig {
  const bucket = process.env.COS_BUCKET
  const region = process.env.COS_REGION
  const accessKeyId = process.env.COS_SECRET_ID
  const secretAccessKey = process.env.COS_SECRET_KEY
  if (bucket === undefined || region === undefined || accessKeyId === undefined || secretAccessKey === undefined) {
    throw new Error('COS_BUCKET, COS_REGION, COS_SECRET_ID, COS_SECRET_KEY must be set (e.g. in .env.development)')
  }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    let r = await ws.execute('ls /cos/')
    console.log('ls /cos/:\n' + r.stdoutText)

    r = await ws.execute("find /cos/ -name '*.json' | head -n 5")
    console.log('find *.json:\n' + r.stdoutText)

    const plan = await ws.execute('grep -m 1 mirage /cos/data/example.jsonl', { provision: true })
    ...
    // dispatch-level stat.
    console.log(`=== metadata overlay on /cos/data/example.jsonl ===`)
    const metaRes = await ws.execute(
      `chmod 640 "/cos/data/example.jsonl" && chown 500:dev "/cos/data/example.jsonl" && touch -t 202601021530 "/cos/data/example.jsonl"`,
    )
    console.log(`  chmod/chown/touch exit=${String(metaRes.exitCode)}`)
    const metaSt = (await ws.dispatch('stat', `/cos/data/example.jsonl`)) as FileStat
