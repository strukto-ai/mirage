...
PYEOF`,
  ...
  const r2 = await ws.shell('python3 /ram/python.py alice bob')
...
PYEOF`,
  ...
  const r3 = await ws.shell('cat /ram/data.jsonl | python3 /ram/count.py')
...
PYEOF`,
  ...
  const r4 = await ws.shell('python3 /ram/python.py')
  ...
  const r5 = await ws.shell('python3 /ram/missing.py')
