...
function configFromEnv(): ScalewayConfig {
  ...
  if (bucket === undefined || region === undefined || accessKeyId === undefined || secretAccessKey === undefined) {
    throw new Error('SCW_BUCKET, SCW_REGION, SCW_ACCESS_KEY, SCW_SECRET_KEY must be set (e.g. in .env.development)')
  }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const metaSt = (await ws.dispatch('stat', `/scw/data/example.jsonl`)) as FileStat
