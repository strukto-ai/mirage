...
async function main(): Promise<void> {
  ...
  try {
    ...
    let r = await ws.shell('ls /oss/')
    console.log('ls /oss/:\n' + r.stdoutText)

    r = await ws.shell("find /oss/ -name '*.json' | head -n 5")
    console.log('find *.json:\n' + r.stdoutText)

    const plan = await ws.shell('grep -m 1 mirage /oss/data/example.jsonl', { provision: true })
    ...
    // chmod/chown/touch never hit the OSS API: attrs land in the
    // workspace namespace (durable, snapshot-captured) and merge into
    // dispatch-level stat.
    console.log(`=== metadata overlay on /oss/data/example.jsonl ===`)
    const metaRes = await ws.shell(
      `chmod 640 "/oss/data/example.jsonl" && chown 500:dev "/oss/data/example.jsonl" && touch -t 202601021530 "/oss/data/example.jsonl"`,
    )
    console.log(`  chmod/chown/touch exit=${String(metaRes.exitCode)}`)
    const metaSt = (await ws.dispatch('stat', `/oss/data/example.jsonl`)) as FileStat
