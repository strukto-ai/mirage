...
async def main():
    ...
    print("=== ls -lh /ds/ ===")
    r = await ws.shell("ls -lh /ds/")
    print(await r.stdout_str())

    print("=== tree -L 2 /ds/ ===")
    r = await ws.shell("tree -L 2 /ds/")
    print((await r.stdout_str())[:400])

    # ── stat ────────────────────────────────────────────
    print("\n=== stat /ds (root) ===")
    r = await ws.shell("stat /ds")
    print(f"  {(await r.stdout_str()).strip()}")

    print("\n=== stat /ds/README.md ===")
    r = await ws.shell("stat /ds/README.md")
    print(f"  {(await r.stdout_str()).strip()}")
    ...
    # ── find ────────────────────────────────────────────
    print("\n=== find /ds/ -name '*.md' ===")
    r = await ws.shell("find /ds/ -name '*.md'")
    print(await r.stdout_str())

    print("=== find /ds/ -type d ===")
    r = await ws.shell("find /ds/ -type d")
    print(await r.stdout_str())
    ...
    r = await ws.shell("find /ds/ -name '*.parquet' | wc -l")
    ...
    r = await ws.shell("wc -l /ds/README.md")
    print(f"  {(await r.stdout_str()).strip()}")

    print("=== head -c 200 /ds/README.md (byte range) ===")
    r = await ws.shell("head -c 200 /ds/README.md")
    print(await r.stdout_str())
    ...
    r = await ws.shell("grep -i license /ds/README.md")
    print(f"  {(await r.stdout_str()).strip()}")

    # ── pipelines ───────────────────────────────────────
    print("\n=== find /ds/ -name '*.parquet' | sort | head -n 3 ===")
    r = await ws.shell("find /ds/ -name '*.parquet' | sort | head -n 3")
    print(await r.stdout_str())

    print("=== cat /ds/README.md | grep -i tag | head -n 5 ===")
    r = await ws.shell("cat /ds/README.md | grep -i tag | head -n 5")
    print(await r.stdout_str())
    ...
    r = await ws.shell("grep -q parquet /ds/README.md && echo 'found'")
    print(f"  stdout: {(await r.stdout_str()).strip()}  exit: {r.exit_code}")

    print("=== grep -q nope /ds/README.md || echo 'absent' ===")
    r = await ws.shell("grep -q nope /ds/README.md || echo 'absent'")
    print(f"  stdout: {(await r.stdout_str()).strip()}  exit: {r.exit_code}")
    ...
    print("\n=== quoting / $(...) / env vars ===")
    await ws.shell("export TGT=/ds/README.md")
    r = await ws.shell('cat "$TGT" | head -n 2')
