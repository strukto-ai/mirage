...
REMOTE_SCRIPT = Path(__file__).parent / "remote" / "fuse.py"


def main():
    daytona = Daytona(DaytonaConfig(api_key=os.environ["DAYTONA_API_KEY"]))

    print("=== building image (debian-slim + git + fuse3 + mirage-ai[s3]) ===")
    ...
    try:
        ...
        print("=== sandbox FUSE probe ===")
        print(probe.result.rstrip())
        print()
