...
async def main():
    ...
    output = await (
        await ws.shell("grep mirage /r2/data/example.jsonl")).stdout_str()
    ...
    output = await (
        await
        ws.shell("grep -m 1 mirage /r2/data/example.jsonl")).stdout_str()
    lines = output.strip().splitlines() if output.strip() else []
    print(f"  Matches: {len(lines)}")
    print(f"  Stats: {ops_summary()}")

    print("\n--- grep mirage /r2/data/example.jsonl | wc -l ---")
    result = await ws.shell("grep mirage /r2/data/example.jsonl | wc -l")
    print(f"  Count: {(await result.stdout_str()).strip()}")
    print(f"  Exit code: {result.exit_code}")
    print(f"  Stats: {ops_summary()}")

    print("\n--- grep mirage /r2/data/example.jsonl | head -n 3 ---")
    result = await ws.shell("grep mirage /r2/data/example.jsonl | head -n 3")
    ...
    result = await ws.shell("grep queue-operation /r2/data/example.jsonl"
                            " | grep -v error | head -n 2 | cut -d , -f 1")
    print(f"  Output:\n    {(await result.stdout_str()).strip()}")

    result_full = await ws.shell("grep queue-operation /r2/data/example.jsonl"
    ...
    print("\n--- rg -l mirage /r2/data ---")
    output = await (await ws.shell("rg -l mirage /r2/data")).stdout_str()
    lines = output.strip().splitlines() if output.strip() else []
    print(f"  Files: {lines}")
    print(f"  Stats: {ops_summary()}")
    ...
    print("--- jq .metadata ---")
    result = await ws.shell("jq .metadata /r2/data/example.json")
    ...
    print("\n--- jq: total budget ---")
    result = await ws.shell("jq .metadata.total_budget /r2/data/example.json")
    print(f"  {(await result.stdout_str()).strip()}")

    print("\n--- jq: compact JSON row ---")
    result = await ws.shell("jq -c .metadata /r2/data/example.json")
    print(f"  {(await result.stdout_str()).strip()}")

    print("\n--- jq: departments pretty json ---")
    result = await ws.shell("jq .departments /r2/data/example.json")
