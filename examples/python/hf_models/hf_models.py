...
async def main():
    ...
    print("=== tree /m/ ===")
    r = await ws.shell("tree /m/")
    print(await r.stdout_str())

    # ── stat (root + small + huge file) ─────────────────
    print("\n=== stat /m/config.json ===")
    r = await ws.shell("stat /m/config.json")
    ...
    r = await ws.shell("stat -c '%s' /m/model.safetensors")
    ...
    r = await ws.shell("wc -l /m/config.json")
    print(f"  {(await r.stdout_str()).strip()}")

    # ── jq: introspect the architecture ─────────────────
    print("\n=== jq .model_type /m/config.json ===")
    r = await ws.shell("jq .model_type /m/config.json")
    print(f"  {(await r.stdout_str()).strip()}")
    ...
    r = await ws.shell("head -n 30 /m/README.md")
    print(await r.stdout_str())

    print("=== grep -ic license /m/README.md ===")
    r = await ws.shell("grep -ic license /m/README.md")
    ...
    # ── find variants ───────────────────────────────────
    print("\n=== find /m/ -name '*.json' ===")
    r = await ws.shell("find /m/ -name '*.json'")
    print(await r.stdout_str())

    print("=== find /m/ -name '*.safetensors' ===")
    r = await ws.shell("find /m/ -name '*.safetensors'")
    print(await r.stdout_str())

    print("=== find /m/ -type f -size +1M ===")
    r = await ws.shell("find /m/ -type f -size +1M")
    print(await r.stdout_str())

    # ── pipelines ───────────────────────────────────────
    print("=== find /m/ -name '*.json' | sort | head ===")
    r = await ws.shell("find /m/ -name '*.json' | sort | head")
    print(await r.stdout_str())

    print("=== cat /m/config.json | grep -c ':' ===")
    r = await ws.shell("cat /m/config.json | grep -c ':'")
    ...
    print("\n=== quoting + $(...) ===")
    await ws.shell("export CFG=/m/config.json")
    r = await ws.shell('jq .hidden_size "$CFG"')
    print(f'  jq .hidden_size "$CFG": {(await r.stdout_str()).strip()}')

    r = await ws.shell("cat $(echo /m/config.json) | jq .num_hidden_layers")
