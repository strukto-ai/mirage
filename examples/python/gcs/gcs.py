...
async def main():
    # ── discover structure ────────────────────────────
    print("=== ls /gcs/ ===")
    r = await ws.shell("ls /gcs/")
    print(await r.stdout_str())

    print("=== ls /gcs/data/ ===")
    r = await ws.shell("ls /gcs/data/")
    print(await r.stdout_str())

    # ── tree ──────────────────────────────────────────
    print("=== tree /gcs/ ===")
    r = await ws.shell("tree /gcs/")
    print(await r.stdout_str())

    # ── stat (directory prefix + file) ──────────────────
    print("=== stat /gcs/data (directory prefix) ===")
    r = await ws.shell("stat /gcs/data")
    print(f"  {(await r.stdout_str()).strip()}")

    print("\n=== stat /gcs/data/example.json ===")
    r = await ws.shell("stat /gcs/data/example.json")
    print(f"  {(await r.stdout_str()).strip()}")

    # ── cat json ──────────────────────────────────────
    print("\n=== cat /gcs/data/example.json | head -n 10 ===")
    r = await ws.shell("cat /gcs/data/example.json | head -n 10")
    print(await r.stdout_str())

    # ── head / tail on jsonl ──────────────────────────
    print("=== head -n 3 /gcs/data/example.jsonl ===")
    r = await ws.shell("head -n 3 /gcs/data/example.jsonl")
    print((await r.stdout_str())[:300])

    print("\n=== tail -n 2 /gcs/data/example.jsonl ===")
    r = await ws.shell("tail -n 2 /gcs/data/example.jsonl")
    print((await r.stdout_str())[:300])

    # ── wc ────────────────────────────────────────────
    print("\n=== wc -l /gcs/data/example.jsonl ===")
    r = await ws.shell("wc -l /gcs/data/example.jsonl")
    print(f"  {(await r.stdout_str()).strip()}")

    # ── grep ──────────────────────────────────────────
    print("\n=== grep -c mirage /gcs/data/example.jsonl ===")
    r = await ws.shell("grep -c mirage /gcs/data/example.jsonl")
    print(f"  count: {(await r.stdout_str()).strip()}")

    print("\n=== grep mirage /gcs/data/example.jsonl | head -n 3 ===")
    r = await ws.shell("grep mirage /gcs/data/example.jsonl | head -n 3")
    ...
    # ── find ──────────────────────────────────────────
    print("\n=== find /gcs/ -name '*.json' ===")
    r = await ws.shell("find /gcs/ -name '*.json'")
    print(await r.stdout_str())

    print("=== find /gcs/ -name '*.parquet' ===")
    r = await ws.shell("find /gcs/ -name '*.parquet'")
    print(await r.stdout_str())
    ...
    r = await ws.shell("cat /gcs/data/example.jsonl | grep queue | head -n 3")
    lines = (await r.stdout_str()).strip().splitlines()
    print(f"  got {len(lines)} lines (expected 3)")

    print("\n=== grep -q && echo (barrier VALUE) ===")
    r = await ws.shell('grep -q queue /gcs/data/example.jsonl && echo "found"')
    ...
    r = await ws.shell("cat /gcs/data/nonexistent_file 2>&1 | head -n 1")
    print(f"  stdout: {(await r.stdout_str()).strip()}")
    print(f"  exit: {r.exit_code}")
    ...
    r = await ws.shell("cat /gcs/data/nonexistent_file | cat")
    ...
    r = await ws.shell("cat /gcs/data/nonexistent_file 2>&1 | cat")
    ...
    r = await ws.shell("sleep 0 & cat /gcs/data/example.json | head -n 1")
    print(f"  stdout: {(await r.stdout_str()).strip()}")
    ...
    # dispatch-level stat.
    print("=== metadata overlay on /gcs/data/example.jsonl ===")
