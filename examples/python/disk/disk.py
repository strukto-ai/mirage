...
async def main() -> None:
    ...
    result = await ws.shell("cat /data/example.json")
    print(await result.stdout_str())

    print("=== head -n 3 /data/example.jsonl ===")
    result = await ws.shell("head -n 3 /data/example.jsonl")
    print(await result.stdout_str())

    print("=== tail -n 2 /data/example.jsonl ===")
    result = await ws.shell("tail -n 2 /data/example.jsonl")
    print(await result.stdout_str())

    print("=== wc /data/example.json ===")
    result = await ws.shell("wc /data/example.json")
    print(await result.stdout_str())

    print("=== stat /data/example.json ===")
    result = await ws.shell("stat /data/example.json")
    print(await result.stdout_str())
    ...
    result = await ws.shell("find /data/ -name '*.json'")
    print(await result.stdout_str())

    print("=== grep example /data/example.json ===")
    result = await ws.shell("grep example /data/example.json")
    print(await result.stdout_str())

    print("=== rg example /data/example.json ===")
    result = await ws.shell("rg example /data/example.json")
    print(await result.stdout_str())
    ...
    result = await ws.shell('jq ".company" /data/example.json')
    print(await result.stdout_str())

    print("=== basename /data/example.json ===")
    result = await ws.shell("basename /data/example.json")
    print(await result.stdout_str())

    print("=== dirname /data/example.json ===")
    result = await ws.shell("dirname /data/example.json")
    print(await result.stdout_str())

    print("=== sort /data/example.jsonl ===")
    result = await ws.shell("sort /data/example.jsonl")
    print(await result.stdout_str())

    print("=== cp /data/example.json /data/copy.json ===")
    await ws.shell("cp /data/example.json /data/copy.json")
    result = await ws.shell("ls /data/")
    print(await result.stdout_str())

    print("=== mv /data/copy.json /data/renamed.json ===")
    await ws.shell("mv /data/copy.json /data/renamed.json")
    result = await ws.shell("ls /data/")
    print(await result.stdout_str())

    print("=== rm /data/renamed.json ===")
    await ws.shell("rm /data/renamed.json")
    result = await ws.shell("ls /data/")
    print(await result.stdout_str())
