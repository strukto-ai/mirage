...
async def main():
    ...
    probe = await ws.shell("ls /supabase/")
    ...
    result = await ws.shell("grep mirage /supabase/example.jsonl | wc -l")
    print(f"  Count: {(await result.stdout_str()).strip()}")
    print(f"  Exit code: {result.exit_code}")
    print(f"  Stats: {ops_summary()}")

    print("\n--- grep mirage /supabase/example.jsonl | head -n 3 ---")
    result = await ws.shell("grep mirage /supabase/example.jsonl | head -n 3")
    ...
    output = await (await ws.shell("rg -l mirage /supabase")).stdout_str()
    lines = output.strip().splitlines() if output.strip() else []
    print(f"  Files: {lines}")
    print(f"  Stats: {ops_summary()}")
    ...
    print("--- jq .metadata ---")
    result = await ws.shell("jq .metadata /supabase/example.json")
    ...
    print("\n--- jq: total budget ---")
    result = await ws.shell("jq .metadata.total_budget /supabase/example.json")
    print(f"  {(await result.stdout_str()).strip()}")

    print("\n--- jq: compact JSON row ---")
    result = await ws.shell("jq -c .metadata /supabase/example.json")
    print(f"  {(await result.stdout_str()).strip()}")

    print("\n--- jq: departments pretty json ---")
    result = await ws.shell("jq .departments /supabase/example.json")
