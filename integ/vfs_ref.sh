#!/usr/bin/env bash
# `mounts: {vfs: <ref>}` battery: a workspace mounts a backend that
# lives in a file on disk, with no host program calling register_vfs,
...
# the mount, and a load given the config again gets the live VFS.
#
# Usage: vfs_ref.sh "<py-cli>" "<ts-cli>"
set -uo pipefail
...
# its own project.
RUN="$(mktemp -d "$HERE/.vfs-ref.XXXXXX")"
trap 'rm -rf "$RUN"' EXIT
...
mounts:
  /wiki:
    vfs: $ref_file:WikiVFS
YML
  if [ "$with_feed" == "1" ]; then
    cat >> "$path" <<YML
  /feed:
    vfs: $ref_file:FeedVFS
YML
  fi
}
...
echo "== python =="
export MIRAGE_HOME="/tmp/vfs-ref-home-py"
rm -rf "$MIRAGE_HOME"
$PY_CLI daemon stop >/dev/null 2>&1 </dev/null || true
freeport
probe "$PY_CLI" py py "../fixtures/vfs/wiki_backend.py"
$PY_CLI daemon stop >/dev/null 2>&1 </dev/null || true

echo "== typescript =="
export MIRAGE_HOME="/tmp/vfs-ref-home-ts"
rm -rf "$MIRAGE_HOME"
$TS_CLI daemon stop >/dev/null 2>&1 </dev/null || true
freeport
probe "$TS_CLI" ts mjs "../fixtures/vfs/wiki_backend.mjs"
probe "$TS_CLI" ts ts "../fixtures/vfs/wiki_backend.ts"
$TS_CLI daemon stop >/dev/null 2>&1 </dev/null || true
