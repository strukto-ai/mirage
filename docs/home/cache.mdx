...
Under `ConsistencyPolicy.ALWAYS`, cached file bytes are checked against live backend metadata, bypassing the index. A changed fingerprint triggers a fresh read; a deleted file is evicted. `LAZY` continues to trust cached bytes.

The check happens wherever cached bytes are served, not only for the files a command names: a recursive walk, a glob and a cross-mount fan-out each revalidate every file they read. That costs one backend stat per file, which is the price of the guarantee.
