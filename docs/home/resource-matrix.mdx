...
All S3-compatible resources accept `key_prefix` in Python and `keyPrefix` in
TypeScript to expose only one bucket subtree as the mount root.
...
| SharePoint | read, write | [SharePoint](/home/setup/sharepoint) | [<Icon icon="python" />](/python/resource/sharepoint) | Document libraries through Microsoft Graph; `site`, `drive`, and `key_prefix` can scope a mount to one folder subtree |
