...
<div className="max-w-none text-[15px] leading-7 mb-4">
  <p className="my-0">
    Mount your sources, then run shell commands across them. The same <code>echo</code>, <code>ls</code>, <code>grep</code> work against an in-memory mount, <Icon icon="aws" /> S3, <Icon icon="slack" /> Slack, etc.
  </p>
</div>
...
<Tabs>
  <Tab title="Python" icon="/images/python-logo.svg">
    ...
    ws = Workspace({
        "/data": Mount(RAMVFS(), mode=MountMode.WRITE),
        "/s3": S3VFS(S3Config(bucket="my-bucket")),
        "/slack": SlackVFS(SlackConfig(token=os.environ["SLACK_BOT_TOKEN"])),
    })

    # Same shell vocabulary, three different backends.
    await ws.shell('echo "hello mirage" > /data/hello.txt')
    await ws.shell("ls /s3/reports/")
    result = await ws.shell('grep -r "release" /slack/channels/eng__C04QX')
  ...
  <Tab title="TypeScript" icon="/images/typescript-logo.svg">
    ...
    const ws = new Workspace({
      '/data': new Mount(new RAMVFS(), { mode: MountMode.WRITE }),
      '/s3': new S3VFS({ bucket: 'my-bucket' }),
      '/slack': new SlackVFS({ token: process.env.SLACK_BOT_TOKEN! }),
    })

    // Same shell vocabulary, three different backends.
    await ws.shell('echo "hello mirage" > /data/hello.txt')
    await ws.shell('ls /s3/reports/')
    const res = await ws.shell('grep -r "release" /slack/channels/eng__C04QX')
...
<Tabs>
  <Tab title="Python" icon="/images/python-logo.svg">
    ...
    slack = SlackVFS(SlackConfig(token="xoxb-..."))
    github = GitHubVFS(
        GitHubConfig(token="github_pat_..."),
        owner="strukto-ai",
        repo="mirage",
    )
    linear = LinearVFS(LinearConfig(api_key="lin_api_..."))
  ...
  <Tab title="TypeScript" icon="/images/typescript-logo.svg">
    ...
    const slack = new SlackVFS({ token: process.env.SLACK_BOT_TOKEN! })
    const github = await GitHubVFS.create({
      token: process.env.GITHUB_TOKEN!,
      owner: 'strukto-ai',
      repo: 'mirage',
    })
    const linear = new LinearVFS({ apiKey: process.env.LINEAR_API_KEY! })
    ...
    <div className="max-w-none text-[15px] leading-7 mt-4 mb-4">
      <p className="my-0">
        Runnable source: <a href="https://github.com/strukto-ai/mirage/blob/main/examples/typescript/agents/openai/multi_vfs_agent.ts" target="_blank" rel="noopener noreferrer"><code>examples/typescript/agents/openai/multi_vfs_agent.ts</code></a>, the same shell-tool pattern over Slack and S3.
      </p>
    </div>
  </Tab>
</Tabs>
...
<AccordionGroup>
  <Accordion title="Do I need FUSE?">
    No. Mirage runs the workspace in-process: <code>ws.shell(...)</code> parses and dispatches commands without mounting anything on the host. FUSE is an optional surface if you also want host tools (editors, language servers, <code>rg</code>) to see the workspace.
  </Accordion>
  ...
  <Accordion title="Is it sandboxed?">
    The shell only sees your mounts: no arbitrary host filesystem, no shelling out to host binaries. For untrusted code you'd still want a real sandbox (Daytona, E2B, Modal); Mirage embeds inside those rather than replacing them.
  </Accordion>
  ...
  <Accordion title="Python or TypeScript?">
    Both. The Python package is the reference implementation; the TypeScript SDK (<code>@struktoai/mirage-node</code>) ships the same <code>Workspace</code>/<code>execute</code> surface and most mounts. Some agent integrations land on Python first.
  </Accordion>
  <Accordion title="What if a VFS I want isn't supported yet?">
    VFS types are pluggable. The <a href="/python/vfs/new">"Add a VFS"</a> guide walks through the read/write/stat surface a new backend implements. PRs welcome.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  <Card title="Python Quickstart" icon="python" href="/python/quickstart">
    Create a workspace, mount your sources, and run shell commands in minutes.
  </Card>
  ...
  <Card title="VFS Matrix" icon="grid-2" href="/home/vfs-matrix">
    Compare VFS types by mount mode, setup path, and common use cases.
  </Card>
</CardGroup>
