...
# in-process duplicate (shares remote VFS, restores local content fresh)
cp = await ws.copy()
```
...
`copy` also keeps the workspace's profiles, default profile, and command limits. Policy objects and the route policy are not copied; add them to the copy again if it needs them.
...
<Columns cols={2}>
  <Card title="Captured" icon="check">
    - Mount configs (creds redacted; restore via `mounts=` override)
...
On the first `dispatch` or `shell` after `load`, Mirage stats every fingerprinted path against the live source in parallel. If any path's live fingerprint differs from the recorded one, the workspace raises `ContentDriftError`:

```python
try:
    await ws.shell("cat /s3/data.csv")
...
| **Index cache** (`VFS._index`) | `FileStat` entries for listings | ❌ rebuilt lazily after load |
...
Remote drift detection is opt-in per VFS through `SUPPORTS_SNAPSHOT` in Python and `supportsSnapshot` in TypeScript. A working adapter must also attach a fingerprint to each read record; a revision is optional and enables pinned replay.

Legend: ✅ = implemented · 🟡 = VFS opts in, but recorded reads do not yet carry the fingerprint · ❌ = live-only · — = unavailable in that runtime.

### Remote VFS

| VFS family | Python | TS Node | TS Browser | Revision pin | Notes |
| --- | :---: | :---: | :---: | :---: | --- |
...
| Hugging Face mounts | 🟡 | 🟡 | — | ❌ | mounts opt in, but read records do not yet include the stat fingerprint. |
| Nextcloud | 🟡 | — | — | ❌ | The VFS opts in, but read records do not yet include the WebDAV ETag. |
| GitHub | ✅ | ✅ | ✅ | ❌ | Uses the git blob sha; a commit pin is not wired. |
| Google Drive | ❌ | ❌ | ❌ | ❌ | Current reads are live-only; revision-aware replay is not wired yet. |
| Other remote mounts | ❌ | ❌ | ❌ | ❌ | Snapshot restores their config/state, then reads current remote content. |

### Local State

RAM, Disk, and Redis serialize their VFS state into the snapshot. They do not need a remote drift check or revision pin: replay restores the captured state directly. Credentials and connection details remain redacted and may require a VFS override at load time.
...
``mount.revisions`` — no per-VFS hook required. If your backend
