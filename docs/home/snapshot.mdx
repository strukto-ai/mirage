...
Both bypass every cache on the way down — the positive and negative index entries, the warm file cache — so the answer always reflects one fresh backend call (two for Google Drive, which resolves a path to a file id before it can ask that id's metadata). `read_with_identity` stamps the bytes it just read from that same response, never a second round trip; `live_identity` is the standalone probe for a pre-write check or a post-write restamp.
