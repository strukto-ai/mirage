...
| Supabase | 📝 | ❌ | 📝 | ❌ | n/a yet | TODO(snapshot-supabase). |
...
### Live‑Only (No Stable Revision Today)

These mounts snapshot config + history but **not content**. Reads at load time serve current state. A `live-only mount(s) opt out of snapshot replay` warning lists them.

| Resource | Why live-only | Could change? |
| --- | --- | --- |
| Gmail | No stable per-message revision for body edits. | Unlikely. |
| Slack | Messages can be edited in place; no public revision marker. | Unlikely. |
| Linear | Issue history exists, but no per-field revision token. | Possible (`updatedAt` could fingerprint). |
| Notion | `last_edited_time` flickers on unrelated edits; would yield false-positive drift. | Possible once block-level revision IDs are stable. |
| Discord | Same shape as Slack. | Unlikely. |
| Trello, Telegram, Postgres, MongoDB, SSH | Live state is the source of truth; snapshotting bytes would be misleading. | Out of scope. |
...
- **First-op latency.** The drift check fires on the first async entry point and issues one `head_object` per recorded read in parallel (`asyncio.gather`). On large recorded sessions, expect a one-time RTT spike.
- **Manifest format breaks.** Snapshots from before [#32](https://github.com/strukto-ai/mirage/pull/32) are not load-compatible: the manifest schema (revision field, `FINGERPRINTS`, `LIVE_ONLY_MOUNTS`) changed.
