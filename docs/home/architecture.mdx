...
`stopping` until its runner and tracked cleanup settle. Parent cancellation
...
admission. `details: none` redacts command text, cwd and failure details. Control
also requires metadata visibility. Inline permissions only narrow these grants.
...
| Monty, both SDKs | Managed worker execution | `await mirage_run(argv, input, cwd)`; returns text stdout, stderr and returncode |
| Pyodide, TypeScript | Managed execution | `subprocess.run(argv, ...)` in a shared-memory worker |
...
gets an explicit unsupported error. Pyodide supports `run` with argv, input,
PIPE/DEVNULL/inherited output, text conversion, cwd and check; it does not supply
`Popen`, PTYs, shell=True, or the remaining Popen options. Its explicit env follows
Mirage's environment-overlay convention. Provider adapters may buffer their output;
