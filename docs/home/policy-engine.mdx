...
policy scripts but no evaluator entry fails at the first decision.

The engine's language is the script's language: `policy: ./policy.py`
needs a python evaluator (monty, or pyodide in TypeScript), while
`policy: ./policy.js` runs on `quickjs`, so a JS-only world scripts its
policy in JavaScript with the same ctx and verdict contract.
