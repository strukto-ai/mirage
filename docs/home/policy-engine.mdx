...
The payload round-trips through JSON (`ExecuteContext.from_dict`), so you
...
In code, the same two arms are the policy vocabulary's own `Route` and
`Deny` actions; the dict is their wire form, the only shape a script can
return from inside the evaluator sandbox:

```python
from mirage import Deny, ExecuteContext, Route

def policy(ctx: ExecuteContext) -> Route | Deny | None:
    if any(p.startswith("/prod/") for p in ctx.commands[0].paths):
        return Deny("writes under /prod are blocked")
    return Route("docker") if "/jobs/" in ctx.line else None
```

Under the hood `policy=` is not a separate system: it registers on the
workspace's policy chain as its `pre_execute` hook. Any policy passed via
`Workspace(policies=[...])` can override `pre_execute` and answer with the
same `Route`/`Deny` arms; a Deny refuses the line, and the first Route wins
placement.
...
```python
from mirage import ExecuteContext
