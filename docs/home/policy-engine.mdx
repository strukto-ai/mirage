...
`route.py` is never imported or called as a module: the workspace hands
its source text to the **policy engine**, the first runtime in the
`runtimes` list with the evaluator capability (`monty` in both languages,
`pyodide` in TypeScript). The engine evaluates it per line with `ctx`
bound as a global, and the last expression comes back as the verdict. In
the config above the engine is `monty`, the same entry that serves
`python3`. If a config carries route scripts but no entry has the
capability, the first decision fails with a `RoutingDecisionError` telling
you to add one.
