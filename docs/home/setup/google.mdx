...
icon: google
description: Set up Google OAuth2 credentials for Docs, Sheets, Slides, Drive, and Gmail mounts.
---

## Overview

The Google mounts use OAuth2 with a refresh token to authenticate against:
...
For personal use with a published app, you do the auth flow **once** and it works indefinitely. The `TokenManager` in the VFS automatically uses the refresh token to obtain fresh access tokens (which expire hourly) behind the scenes.
...
For Python configuration, see the VFS pages: [Gmail](/python/vfs/gmail), [Drive](/python/vfs/gdrive), [Docs](/python/vfs/gdocs), [Sheets](/python/vfs/gsheets), [Slides](/python/vfs/gslides).
