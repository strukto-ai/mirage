...
title: SharePoint
icon: microsoft
description: Get a Microsoft Graph access token for the SharePoint VFS.
---

The SharePoint VFS talks to Microsoft SharePoint Online through the
...
**not** need a drive id: the VFS discovers sites and document libraries for
...
works for SharePoint because the VFS uses `/sites` and `/sites/{id}/drives`
...
VFS refreshes on `401`, so the mount survives token expiry.
</Note>
...
For Python configuration, see the [Python SharePoint VFS](/python/vfs/sharepoint) guide.
