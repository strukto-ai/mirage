...
description: Set up a Qdrant connection for the Qdrant VFS.
---
...
For document/chunk collections, config field names accept dotted payload paths:
use `group_by=["metadata.source"]`, `basename_fields=["metadata.source"]`,
`name_field="metadata.page"`, and `text_field="page_content"` to expose a
source-document folder with readable page/chunk filenames. TypeScript uses the
camelCase equivalents `groupBy`, `basenameFields`, `nameField`, and `textField`.

See the [Python](/python/vfs/qdrant) and [TypeScript](/typescript/setup/qdrant)
