...
title: Email
icon: envelope
description: Set up IMAP/SMTP credentials for the Email VFS.
---

## Credentials

The Email VFS connects to any email account via IMAP (reading)
and SMTP (sending). You need the server hostnames and a password or
app password.

### 1. Find Your IMAP/SMTP Settings

| VFS    | IMAP Host               | IMAP Port | SMTP Host             | SMTP Port |
| ----------- | ----------------------- | --------- | --------------------- | --------- |
...
not your regular account password. Most providers block regular
...
For Python configuration, see the [Python Email VFS](/python/vfs/email) page.
