...
description: Set up Supabase Storage credentials for the Supabase VFS.
---
...
<Tip>
Supabase requires **path-style** S3 URLs. The Mirage VFS sets `forcePathStyle: true` automatically when you pass `project_ref`.
</Tip>

For Python configuration, see the [Python Supabase VFS](/python/vfs/supabase) guide. For Node and browser wiring, see the [TypeScript Supabase Setup](/typescript/setup/supabase) guide.
