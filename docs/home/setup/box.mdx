...
The Box VFS uses OAuth2 against the [Box v2 API](https://developer.box.com/reference/) to:
...
If you just want to try the VFS against your own Box account, the developer token is the
...
When `BOX_DEVELOPER_TOKEN` is set, the `BoxVFS` skips the OAuth refresh path entirely and
...
```ts
import { BoxVFS } from '@struktoai/mirage-node'
...
```ts
import { BoxVFS } from '@struktoai/mirage-browser'
...
For Python usage (same credentials), see [Box (Python)](/python/vfs/box):

```python
from mirage.vfs.box import BoxConfig, BoxVFS

vfs = BoxVFS(BoxConfig(
