...
every command stops after 600 seconds. Change that with `command_limits` for
the workspace, a mount, or a session profile:

```yaml
command_limits:            # the default for every session
  head:
    max_lines: 500
    timeout_seconds: 30
mounts:
  /data:
    vfs: ram
    mode: WRITE
    command_limits:        # commands that run on /data
      grep:
        max_lines: 50
        on_exceed: error
profiles:
  researcher:
    command_limits:        # sessions created with this profile
      head:
        max_lines: 5000
```
