...
Every factory-built backend estimates read commands out of the box, by
family: whole-file readers (`cat`, `sort`, `md5`, ...) charge the byte
total from `stat`, `head`/`tail` charge a bounded range, `grep`/`rg`
charge a worst-case full read, and metadata commands (`ls`, `find`,
`tree`, ...) charge op counts only. Write commands report
`precision=unknown`. Virtual files whose size cannot be resolved (for
example a rendered `chat.jsonl`) degrade the estimate to
`precision=unknown` while keeping the known byte total as a floor.
