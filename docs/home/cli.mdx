...
| `mirage session create WS [--id NAME] [-m /prefix[:role]]...` | Add a named session (own cwd + env), optionally granting mounts with a role ceiling (`read`/`write`/`exec` or `r`/`rw`/`rwx`). |
