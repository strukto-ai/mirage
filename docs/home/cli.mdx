...
<Note>
  ...
  that need fresh creds. Local mounts (RAM, Disk) restore as-is with
...
| Verb | What it does |
| --- | --- |
| `mirage workspace create FILE [--id NAME]` | Build mounts from YAML, register a workspace, return its id. |
...
Set workspace defaults with `command_limits`, and override individual commands
for a session in `profiles.<name>.command_limits`. Mounts may also declare
command overrides. The order is profile, mount, workspace, command-declared
default, then the built-in default. A selected command entry replaces the
whole limit; an omitted command inherits. `max_lines: null` and
`max_bytes: null` mean unlimited, while zero permits no output.
...
A pipe-only reader uses its profile or workspace default: it does not inherit
upstream mount settings. Thus `cat /data/n.txt | head -n 3000` returns up to
2000 lines normally and all 3000 under `researcher`. Commands spanning several
mounts use the tightest resolved bound across those mounts.

Caps apply to each command's terminal stdout before outputs are combined.
`cat big.txt; echo end` caps `cat` and still prints `end`. Intermediate pipe
stages, file redirects, and captured substitutions retain their full data.
This is a deliberate output-display safeguard beyond GNU behavior; it does
not change how much data `cat big.txt | wc -l` counts.

Each entry accepts `max_lines`, `max_bytes`, `timeout_seconds`, and `on_exceed`.
`truncate` retains the bounded output and adds a command-attributed stderr
notice without changing the command's status. `error` discards that command's
stdout and exits 1, before `&&`, `||`, `$?`, and `PIPESTATUS` use its status.
A timeout exits 124. Exact-fit output has no truncation notice.
Without configuration, `cat`, `grep`, `rg`, `head`, and `tail` cap at 2000 lines.

Independent policy ceilings still apply. A policy's `post_execute` limit
bounds the complete response; it is separate from command defaults. A runtime
executing an opaque whole shell line exposes one invocation, so its cap is
applied to the returned response rather than to internal pipeline stages.
See [Output Limits](/python/quickstart#output-limits) for SDK examples.
