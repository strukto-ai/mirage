...
List methods follow `nextPageToken` to the end by default and print one
JSON response per line; `--page-limit N` stops early. See
[Pagination](/python/resource/gdrive#pagination) for the details and the
reasoning. `gws --help` and `gws gmail --help` list the surface.

```bash
# List labels
gws gmail users labels list --params '{"userId": "me"}'

# Every unread message id, across all pages
gws gmail users messages list --params '{"userId": "me", "q": "is:unread"}' \
  | jq -r '.messages[]?.id'

# Just the first page
gws gmail users messages list --params '{"userId": "me"}' --page-limit 1
