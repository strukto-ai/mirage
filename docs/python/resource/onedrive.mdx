...
config = OneDriveConfig(
    ...
    # cloud=GraphCloud.US_GOV_HIGH,  # a national cloud (default: global)
...
| `cloud`          | No       | National cloud deployment (default `GraphCloud.GLOBAL`)                |
| `graph_base_url` | No       | Full service root, overriding `cloud`                                  |
...
`cloud` selects the Graph deployment. Each is network-isolated with its own
service root, and a token minted for one is rejected by the others, so this is
not a performance knob: pointing a GCC High tenant at the global host fails
outright.

| `GraphCloud` | Service root                             | Microsoft 365 environment |
| ------------ | ---------------------------------------- | ------------------------- |
| `GLOBAL`     | `https://graph.microsoft.com`            | Commercial, and GCC       |
| `US_GOV_HIGH`| `https://graph.microsoft.us`             | GCC High (L4)             |
| `US_GOV_DOD` | `https://dod-graph.microsoft.us`         | DoD (L5)                  |
| `CHINA`      | `https://microsoftgraph.chinacloudapi.cn`| 21Vianet                  |

GCC (moderate) is served by the worldwide endpoint, so it uses `GLOBAL`.

`graph_base_url` is the escape hatch for a root this table cannot name (a
private endpoint, or a test server). It is the full root including the version
segment, for example `https://graph.microsoft.us/v1.0`, and it wins over
`cloud`. Because the root is read from the config rather than a module
constant, two mounts in one process can address different deployments.
