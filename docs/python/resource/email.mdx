...
async def main():
    ...
    r = await ws.execute("email list --folder INBOX --unseen --max 5")
    print(await r.stdout_str())
...
Resource-specific commands wrap the common IMAP/SMTP actions. Mirage keeps
the message body as files in the tree, so these commands take `--uid` and
`--folder` flags and return JSON rather than opening an interactive editor.

For agents familiar with the [himalaya](https://github.com/pimalaya/himalaya)
CLI, each command also has a himalaya-style alias that dispatches to the same
handler (identical flags and output):

| Canonical      | himalaya alias             |
| -------------- | -------------------------- |
| `email list`   | `himalaya envelope list`   |
| `email read`   | `himalaya message read`    |
| `email send`   | `himalaya message send`    |
| `email reply`  | `himalaya message reply`   |
| `email forward`| `himalaya message forward` |
...
email reply --uid 12345 --folder INBOX --body "Acknowledged" --all
```
...
```bash
email list --folder INBOX --unseen --max 10
```
