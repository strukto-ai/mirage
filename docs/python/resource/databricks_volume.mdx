...
| `token` | required | Bearer token, held as a secret and redacted in snapshots. |
...
The token is a `SecretStr`: it never appears in a `repr` and a snapshot
records it as `<REDACTED>`. There is no `profile` field and no environment
discovery — whatever your deployment uses to obtain a token, resolve it and
pass the string. See [Databricks Volume Setup](/python/setup/databricks).
...
ws = await Workspace.load(
    "snap.tar",
    resources={"/dbx/": DatabricksVolumeResource(config)},
)
```

## Expired tokens

MIRAGE sends the configured token on every call and never refreshes or retries
it. A refusal raises `DatabricksVolumeAuthError` out of an op call
(`ws.readdir`, `ws.stat`, `ws.dispatch`); through `ws.execute` it arrives as a
nonzero `IOResult` instead, because the executor answers a failed shell line
the way a shell does. Either way, recovery is a new resource built from a new
config. See the [setup guide](/python/setup/databricks) for both shapes.

## Databricks Apps

Inside a Databricks App, keep Mirage in-process and let the app's own auth
supply the token — typically the token of the signed-in user, resolved per
request:

```python
# model_validate, not model_copy(update=...): a copy skips validation, so
# the token would stay a plain str and its value would show in the repr.
config = DatabricksVolumeConfig.model_validate({
    **config.model_dump(),
    "token": request_user_token(),
})
ws = Workspace({"/dbx/": DatabricksVolumeResource(config)}, mode=MountMode.READ)
```
...
- **Agents in Databricks Apps**: mount a Unity Catalog volume in-process so an
  agent can read and write governed files without FUSE, and reach it with the
