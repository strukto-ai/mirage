...
The config holds no credential. The bearer token arrives through a
`TokenProvider` passed to the constructor, which MIRAGE consults before each
Files API operation and never stores. `StaticTokenProvider` is the only one
MIRAGE ships; see [Databricks Volume Setup](/python/setup/databricks) for
on-behalf-of, service-principal and CLI-profile providers you write yourself.
...
ws = await Workspace.load(
    "snap.tar",
    resources={"/dbx/": DatabricksVolumeResource(config, provider)},
)
```
...
supply the token — typically on behalf of the signed-in user:

```python
resource = DatabricksVolumeResource(config, OnBehalfOfTokenProvider())
ws = Workspace({"/dbx/": resource}, mode=MountMode.READ)
```
...
- **Agents in Databricks Apps**: mount a Unity Catalog volume in-process so an
  agent can read and write governed files without FUSE, and reach it on behalf
  of the signed-in user through a token provider.
