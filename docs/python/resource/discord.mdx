...
Snowflake IDs are **not** embedded in directory or file names. Resource
specific commands resolve IDs via the in-memory `IndexCacheStore`, and
`stat` exposes them in the `extra` dict (see [Finding IDs](#finding-ids)).
Names containing unsafe characters have `/` replaced with `∕` (U+2215).
...
grep hello "/discord/My Server/channels/general/2026-04-04/chat.jsonl"
...
jq -r '.[] | "\(.id) [\(.author.username)] \(.content)"' \
  "/discord/My Server/channels/general/2026-04-04/chat.jsonl"
...
wc -l "/discord/My Server/channels/general/2026-04-04/chat.jsonl"

# Read only recent messages
tail -n 10 "/discord/My Server/channels/general/2026-04-04/chat.jsonl"
...
jq -r '.[] | "\(.author.username): \(.content)"' \
  "/discord/My Server/channels/general/2026-04-04/chat.jsonl" | head -n 20

# Count messages per user
cat "/discord/My Server/channels/general/2026-04-04/chat.jsonl" \
  | jq -r '.[] | .author.username' | sort | uniq -c
```
