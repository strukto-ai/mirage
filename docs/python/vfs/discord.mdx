...
icon: discord
---
The Discord VFS exposes guild, channel, and member data as a virtual
filesystem mounted at some prefix such as `/discord/`.

For token setup, see [Discord Setup](/home/setup/discord).
...
channel, member, and attachment names so VFS specific commands can
...
The Discord VFS uses `IndexCacheStore` (same as RAM/S3/disk/GitHub).
...
async def main():
    ...
    r = await ws.shell(f'ls "{base}/2026-04-04/files/"')
    print(await r.stdout_str())
    r = await ws.shell(
    ...
    r = await ws.shell(f'grep hello "{base}/"')
    print(await r.stdout_str())

    # Search across guild
    r = await ws.shell(f'grep hello "/discord/{guild}/"')
    print(await r.stdout_str())
...
VFS-specific commands require Discord snowflake IDs
