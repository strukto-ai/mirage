...
The Mem0 VFS exposes the memories of a single scope — a `user_id`,
...
vfs = Mem0VFS(config=config)
ws = Workspace({"/memory/": vfs}, mode=MountMode.READ)
```

The VFS is read-only and does not support snapshots. Set exactly one of
...
The VFS uses Mirage's index cache for the virtual tree. The first `ls`
...
async def main() -> None:
    ...
    r = await ws.shell("ls /memory/")
    print(await r.stdout_str())

    r = await ws.shell("grep -r -i 'allergic' /memory/")
    print(await r.stdout_str())

    r = await ws.shell('search --top-k 5 "morning routine" /memory/')
    print(await r.stdout_str())
