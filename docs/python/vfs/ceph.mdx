...
---
The Ceph VFS is a thin wrapper over the [S3 VFS](/python/vfs/s3).
...
async def main() -> None:
    ...
    r = await ws.shell("find /ceph/ -name '*.json'")
    print(await r.stdout_str())
...
- Ceph reports `VFSName.S3` and routes through the same `core/s3`
