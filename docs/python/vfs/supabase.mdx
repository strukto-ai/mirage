...
---
The Supabase VFS mounts a Supabase Storage bucket at some prefix
...
vfs = SupabaseVFS(config=config)
ws = Workspace({"/supabase": vfs}, mode=MountMode.READ)
```

`SupabaseVFS(config)` takes a `SupabaseConfig` object with the
...
The Supabase VFS maps object keys to virtual paths under the mount
...
The Supabase VFS uses `IndexCacheStore` with `index_ttl = 600`
...
async def main() -> None:
    ...
    r = await ws.shell("ls /supabase/")
    print(await r.stdout_str())

    r = await ws.shell("cat /supabase/documents/data.csv | head -n 10")
    print(await r.stdout_str())

    r = await ws.shell("tree -L 2 /supabase/")
    print(await r.stdout_str())

    r = await ws.shell("find /supabase/ -name '*.pdf'")
    print(await r.stdout_str())

    r = await ws.shell("stat /supabase/avatars/user-001.png")
    print(await r.stdout_str())
...
The Supabase VFS supports the full set of shell commands since it operates
