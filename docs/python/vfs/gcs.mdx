---
title: GCS
description: Mount Google Cloud Storage buckets through Mirage's async S3-compatible VFS layer for Python agents.
icon: google
---
The GCS VFS mounts a Google Cloud Storage bucket at some prefix
...
inheriting all S3 VFS capabilities.
...
from mirage.vfs.gcs import GCSConfig, GCSVFS
...
`GCSVFS(config)` takes a `GCSConfig` object with the bucket name
and HMAC credentials. Both `READ` and `WRITE` modes are supported.

## Filesystem Layout

The GCS VFS maps object keys to virtual paths under the mount
prefix, identical to the S3 VFS. GCS "directories" are
...
The GCS VFS uses `IndexCacheStore` with `index_ttl = 600`
...
The GCS VFS supports the full set of shell commands since it operates
