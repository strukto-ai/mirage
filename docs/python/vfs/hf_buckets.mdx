...
---
The HF Buckets VFS mounts a [Hugging Face Bucket](https://huggingface.co/docs/buckets)
...
ws = Workspace({"/hf": vfs}, mode=MountMode.READ)
```

`HfBucketsVFS(config)` takes an `HfBucketsConfig` object with the
bucket in `namespace/bucket-name` form plus an optional access token.
Both `READ` and `WRITE` modes are supported out of the box.

## Filesystem Layout

The HF Buckets VFS maps bucket object keys to virtual paths under
the mount prefix.
...
The HF Buckets VFS uses `IndexCacheStore` with `index_ttl = 600`
...
async def main() -> None:
    ...
    r = await ws.shell("cat /hf/data/file.txt")
    print(await r.stdout_str())
    ...
    r = await ws.shell("grep example /hf/data/config.json")
    print(await r.stdout_str())

    r = await ws.shell("stat /hf/data/file.txt")
    print(await r.stdout_str())
...
The HF Buckets VFS supports the full set of shell commands since it operates
