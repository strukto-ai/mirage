...
icon: aws
---
The S3 VFS mounts an Amazon S3 bucket (or any S3-compatible service) at
...
vfs = S3VFS(config)
ws = Workspace({"/s3": vfs}, mode=MountMode.READ)
```

`S3VFS(config)` takes an `S3Config` object with the bucket name and
optional credentials, endpoint, and connection settings. Both `READ` and
`WRITE` modes are supported.

## Filesystem Layout

The S3 VFS maps S3 object keys to virtual paths under the mount prefix.
...
The S3 VFS uses `IndexCacheStore` with `index_ttl = 600` (10 minutes).
...
async def main() -> None:
    ...
    r = await ws.shell("cat /s3/data/file.txt")
    print(await r.stdout_str())

    r = await ws.shell("tree /s3/")
    print(await r.stdout_str())

    r = await ws.shell("find /s3/ -name '*.json'")
    print(await r.stdout_str())

    r = await ws.shell("grep example /s3/data/config.json")
    print(await r.stdout_str())

    r = await ws.shell("stat /s3/data/file.txt")
    print(await r.stdout_str())
...
The S3 VFS supports the full set of shell commands since it operates
...
cat = S3_COMMANDS.require("cat")


async def logged_cat(accessor, paths, texts, opts):
    print("reading", [path.virtual for path in paths])
    return await cat.fn(accessor, paths, texts, opts)


ws.mount("/s3/").register(cat.with_overrides(fn=logged_cat))
```

The builtin definition and other mounts stay unchanged. Registration is a host
application action; an agent can discover and run the resulting `cat` command,
but cannot alter the command table.
