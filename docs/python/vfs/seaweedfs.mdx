...
---
The SeaweedFS VFS is a thin wrapper over the [S3 VFS](/python/vfs/s3).
...
async def main() -> None:
    ws = Workspace({"/seaweedfs/": vfs}, mode=MountMode.READ)
    ...
    r = await ws.shell("cat /seaweedfs/data/config.json")
    print(await r.stdout_str())
