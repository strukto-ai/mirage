...
The SharePoint VFS mounts Microsoft SharePoint Online at some prefix such
as `/sharepoint/`. Unlike the single-drive OneDrive VFS, it exposes the
...
from mirage.vfs.sharepoint import SharePointVFS, SharePointConfig
...
vfs = SharePointVFS(config)
ws = Workspace({"/sharepoint": vfs}, mode=MountMode.WRITE)
```

`SharePointVFS(config)` takes a `SharePointConfig` object with a Microsoft
Graph bearer token. No drive id is required: the VFS discovers sites and
...
With a callable, the VFS refreshes the token on `401`, so long-running
mounts survive token expiry.

`graph_base_url` selects the Graph deployment, and means the same here as for
OneDrive: see [The service root](/python/vfs/onedrive#the-service-root).
...
SharePoint keeps per-file version history. The VFS exposes it the same way
...
This makes the SharePoint VFS `SUPPORTS_SNAPSHOT = True`.

## Cache

The SharePoint VFS caches site, drive, and directory listings via
...
async def main() -> None:
    ...
    r = await ws.shell("ls /sharepoint/Engineering/")
    print(await r.stdout_str())

    await ws.shell(
        "echo 'hello from mirage' > /sharepoint/Engineering/Documents/note.txt")

    r = await ws.shell("cat /sharepoint/Engineering/Documents/note.txt")
    print(await r.stdout_str())

    r = await ws.shell("tree /sharepoint/Engineering/Documents/")
    print(await r.stdout_str())
...
The SharePoint VFS supports the full set of shell commands since it
...
- Use **[OneDrive](/python/vfs/onedrive)** for a single drive (personal
