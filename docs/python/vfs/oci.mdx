...
---
The OCI VFS mounts an Oracle Cloud Infrastructure Object Storage
...
from mirage.vfs.oci import OCIConfig, OCIVFS
...
vfs = OCIVFS(config=config)
ws = Workspace({"/oci": vfs}, mode=MountMode.READ)
```

`OCIVFS(config)` takes an `OCIConfig` object with the bucket name,
...
The OCI VFS maps object keys to virtual paths under the mount
...
The OCI VFS uses `IndexCacheStore` with `index_ttl = 600`
...
async def main() -> None:
    ...
    r = await ws.shell("ls /oci/")
    print(await r.stdout_str())

    r = await ws.shell("cat /oci/data/users.csv | head -n 5")
    print(await r.stdout_str())

    r = await ws.shell("tree /oci/")
    print(await r.stdout_str())

    r = await ws.shell("find /oci/ -name '*.jsonl'")
    print(await r.stdout_str())

    r = await ws.shell("stat /oci/config.json")
    print(await r.stdout_str())
...
The OCI VFS supports the full set of shell commands since it operates
