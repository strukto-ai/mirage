...
The LanceDB VFS exposes a LanceDB table as a virtual filesystem mounted at
...
A group value holding `/` renders as `∕`, and a blank or dot-led value is led
by `⁄`, so every value keeps its own directory that lists and opens, and
