...
icon: google
---
The Google Drive VFS exposes a Google Drive account as a virtual
...
The VFS supports workspace snapshots. Recorded reads capture the
...
The Google Drive VFS uses `IndexCacheStore` (same as Slack,
Gmail, and other mounts). Index entries store folder IDs, file
...
async def main():
    ...
    # Browse a subfolder
    r = await ws.shell("ls /gdrive/Projects/")
    print(await r.stdout_str())

    # Read a regular file
    r = await ws.shell("cat /gdrive/Projects/spec.pdf | head -c 200")
    print(await r.stdout_str())

    # Read a Google Doc title
    r = await ws.shell('jq ".title" /gdrive/Notes/meeting.gdoc.json')
    print(await r.stdout_str())
    ...
    r = await ws.shell('rg "quarterly" /gdrive/Projects/')
    print(await r.stdout_str())

    # Tree view
    r = await ws.shell("tree -L 2 /gdrive/")
    print(await r.stdout_str())
