---
title: Disk
description: Mount a local directory as a Mirage VFS with read/write shell commands and path traversal protection.
icon: hard-drive
---
...
`DiskVFS(root=...)` takes a single `root` path argument pointing to the
