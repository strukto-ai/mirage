...
The Dify VFS exposes completed Dify Knowledge documents as text files
mounted at a prefix such as `/knowledge/`. Each file is assembled from the
document's completed, enabled segments.

For API key setup, see [Dify Setup](/home/setup/dify).
...
vfs = DifyVFS(config=config)
ws = Workspace({"/knowledge/": vfs}, mode=MountMode.READ)
```

The VFS is read-only and does not support snapshots.
...
The VFS uses Mirage's index cache for the virtual tree. Directory listings
...
async def main() -> None:
    ...
    r = await ws.shell("ls /knowledge/")
    print(await r.stdout_str())

    r = await ws.shell("find /knowledge/ -type f | head -n 10")
    print(await r.stdout_str())

    r = await ws.shell('search --method hybrid --top-k 5 "getting started" /knowledge/')
    print(await r.stdout_str())
