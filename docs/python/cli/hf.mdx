...
[`hf_models`](/python/vfs/hf_models),
[`hf_datasets`](/python/vfs/hf_datasets) and
[`hf_spaces`](/python/vfs/hf_spaces) mounts are the read half, and `ls`,
...
ws = Workspace({
    "/m": HfModelsVFS(HfModelsConfig(repo_id="acme/model", token=token)),
    "/work": Mount(RAMVFS(), mode=MountMode.WRITE),
})
ws.register_cli("hf", HF, {"token": token})

await ws.shell("hf auth whoami")
```
...
The Hub VFS need no extra: they speak the Hub API over `aiohttp`, which
mirage already depends on. `mirage-ai[hf-buckets]` is a different product, the
Xet object store behind [HF Buckets](/python/vfs/hf_buckets).
