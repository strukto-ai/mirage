...
Drop-in replacement for camel's `TerminalToolkit`. Same 6 functions, all routed through `Workspace.shell()`:

| Method | Mirage translation |
| --- | --- |
| `shell_exec(id, cmd, block=True)` | `await ws.shell(cmd)` |
| `shell_exec(id, cmd, block=False)` | `await ws.shell(f"{cmd} &")`, uses Mirage's [JobTable](/python/quickstart) |
...
Subclasses camel's `FileToolkit`, inherits all 7 public methods *and* every format writer (PDF, DOCX, JSON, CSV, HTML, ipynb, plain text). Path resolution and IO are overridden to route through Mirage; format writers operate on a tempfile and the result is pushed via `Workspace.shell(f"cat > {path}", stdin=...)`.
...
| Symbol | Purpose |
| --- | --- |
| `MirageTerminalToolkit` | Camel `BaseToolkit` exposing the 6 shell tools, backed by `Workspace.shell`. |
