...
The full write surface works: create, mkdir, rename, in-place writes and
unlink, pinned by `integ/truth_fskit.json`. This depends on
`mirage/fuse/darwin.py`, which declares macFUSE's Darwin-only callbacks
...
<Warning>
  **Two upstream FSKit-shim caveats remain** (macFUSE 5.3.3):
