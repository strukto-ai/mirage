...
| `backend` | Effect |
| --- | --- |
| `workspace` (default) | Lives only inside mirage's own filesystem, reached through the command surface. Nothing is registered with the kernel. |
...
mounts:
  ...
  /logs:
    vfs: disk
    # no backend key: stays on the workspace default, not kernel-mounted
```
...
- **Every mounted VFS must be able to size its files.** See below.
...
<Warning>
  **Treat fskit mounts as read-mostly.** The macFUSE FSKit shim flushes
  pages a file did not already have (a new file, `cp`, `>` truncation) as
  NUL bytes; appends are hit-or-miss. The writer sees no error, and the
  kernel page cache serves the written data back, so the corruption only
  shows in the backing store. Mirage warns at mount time on writable fskit
  mounts (`check_writes`); use `MountBackend.FUSE` for writes. Pinned in
  `integ/fuse/truth_fskit.json` so a shim fix flips the test.
</Warning>
...
<Warning>
  **Size-unknown files read as empty.** There is no `direct_io`, so reads
  stop at the size `stat` reported; a file that stats 0 pre-open reads as
  empty with exit code 0. Mirage warns at mount time, naming the degraded
  mounts (`SIZES_ALWAYS_KNOWN`). Byte stores (ram, disk, redis, s3,
  gridfs) and Linear (size push-down) qualify; for the rest, use
  `MountBackend.FUSE` or scope the fskit mount to a sized subtree.
</Warning>
...
Some mounts (Linear, Trello, Slack, ...) cannot report a file's size
...
Extended attributes over FUSE are the ones the Mirage shell's `getfattr`
and `setfattr` read and write. Mirage stores them in its namespace beside
the symlinks, so every backend takes them.
