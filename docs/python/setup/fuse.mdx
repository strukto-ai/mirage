...
<Warning>
  **FSKit mounts are read-mostly, and write failures are not clean.**
  Measured on macFUSE 5.3.3 / macOS 26 (`integ/fskit.py`): reads, in-place
  writes and `unlink` work; `create`, `mkdir` and `rename` return `ENOSYS`,
  and a failed create **still applies** (the file exists despite the error).
  Anything that creates files (`git clone`, `pip install`, an agent writing
  output) will fail. Use `MountBackend.FUSE` for write workloads, and prefer
  `MountMode.READ` on fskit mounts so the limit surfaces as a clean
  permission error.
</Warning>
...
<Warning>
  **FSKit cannot serve size-unknown resources.** There is no `direct_io`, so
  ...
  stats 0 pre-open would return empty files with exit code 0. Mirage refuses
  such a mount by name at mount time (`SIZES_ALWAYS_KNOWN`). Byte stores
  (ram, disk, redis, s3, gridfs) qualify; API-backed resources need
  `MountBackend.FUSE`, or scope the fskit mount to the byte-store subtree.
</Warning>
