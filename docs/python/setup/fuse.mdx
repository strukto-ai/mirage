...
of the macFUSE kernel extension. Set `fuse_backend` on the mount:
...
- **The mountpoint must be under `/Volumes`.** FSKit refuses anything else.
  Mirage picks a `/Volumes` temp directory automatically when you do not pass
  a path, and rejects an explicit path outside it.
...
default is a trap. `fuse_backend="fskit"` is macOS-only and raises elsewhere.
...
<Warning>
  **FSKit cannot serve size-unknown resources.** FSKit has no `direct_io`
  ...
  backends), `gridfs`. Everything else needs `fuse`. To use FSKit in a
