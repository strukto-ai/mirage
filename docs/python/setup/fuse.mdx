...
refusing a resource that cannot size its files, a live mount with reads
matching their stat size, and each write op with the errno it returns.

TypeScript has no FSKit path at all; `examples/typescript/fuse/fskit.ts`
shows the refusal and the two alternatives (`backend: 'fuse'` there, or the
Python package for a kext-free mount).
