...
| `wasi` | Full CPython compiled to WebAssembly, run in-process on [wasmtime](https://wasmtime.dev) | FUSE-mounted workspace mounts; no host filesystem, no network | No |
...
own build directory plus whatever workspace mounts are FUSE-mounted.

A mount with an active FUSE mountpoint (the `fuse:` key on the mount,
or `add_fuse_mount`) is preopened into the run at the mount's own
path, so `open('/data/f.txt')` inside the code reads and writes the
mount live. That I/O flows through the kernel into the workspace
(namespace, cache, write modes), so the code sees exactly what `cat`
sees and its writes are immediately visible to every command. A mount
at `/` relocates the interpreter build to `/.mirage-wasi`
transparently. Without a FUSE mount, mounts stay invisible: script
files still work (`python3 /data/script.py` reads the script through
the workspace before the run), but file I/O inside the code has
nowhere to land; use `monty` for mount I/O without FUSE.
...
the sandbox (CPython requires it); make it read-only on the host
