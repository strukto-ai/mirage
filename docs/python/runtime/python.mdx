...
Point mirage at the build directory with the `wasi_python` argument, the
yaml `runtime: wasi_python:` key, or the MIRAGE_WASI_PYTHON environment
variable:
...
ws = Workspace({"/data": RAMResource()},
               mode=MountMode.EXEC,
               python_runtime="wasi",
               wasi_python="/path/to/unzipped/build")
```
...
It defaults to the interpreter running mirage; point `local_python`
...
runtime:
  python: local
  local_python: /path/to/venv/bin/python
```
