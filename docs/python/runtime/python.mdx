...
Point mirage at the build directory with a `wasi` entry in the runtime
home map (the `runtime_home` argument, the yaml `runtime: home:` map,
or the MIRAGE_WASI_HOME environment variable):

```python
ws = Workspace({"/data": RAMResource()},
               mode=MountMode.EXEC,
               python_runtime="wasi",
               runtime_home={"wasi": "/path/to/unzipped/build"})
```
...
It defaults to the interpreter running mirage; point the `local` home
entry (or the MIRAGE_LOCAL_HOME environment variable) at another
...
runtime:
  ...
  home:           # optional: where each runtime finds its interpreter
    wasi: /opt/cpython-wasi
    local: /usr/bin/python3.12
...
`home` maps a runtime name to the location of its interpreter or
distribution, in the spirit of JAVA_HOME: a build directory for
`wasi`, an interpreter path for `local`, a distribution URL for
`pyodide` (TypeScript). Only the selected runtime consumes its entry,
so one config stays portable across runtimes and languages; each entry
also falls back to a MIRAGE_&lt;RUNTIME&gt;_HOME environment variable.
`monty` embeds its interpreter and takes no entry.
