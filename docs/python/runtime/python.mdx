...
Monty keeps its normal module boundary: `import subprocess` raises
`ModuleNotFoundError`. Mirage tracks the Monty invocation, but does not inject a
replacement guest API. The TypeScript SDK's Pyodide runtime wires ordinary
Python `subprocess.Popen` and its standard helpers into the workspace.
