...
- The importable stdlib is `asyncio`, `base64`, `binascii`, `collections`, `dataclasses`,
  `datetime`, `functools`, `itertools`, `json`, `math`, `os`, `pathlib`,
  `re`, `sys`, `typing` and `unicodedata`, and each is
...
- Introspection builtins are absent: `dir`, `vars`, `globals`, `locals`,
  `help`, `callable`, `issubclass`, `super`, `eval`, `exec` and
