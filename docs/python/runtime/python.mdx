...
It needs the `wasi` extra plus a CPython WASI build:

```bash
pip install mirage-ai[wasi]
# download and unzip a release from
# https://github.com/brettcannon/cpython-wasi-build/releases
export MIRAGE_WASI_PYTHON=/path/to/unzipped/build
```
...
interpreter busy in the background until it finishes (monty and wasi
run on a worker thread, `local` on a host subprocess). Give untrusted
code a finite workload, or run it behind a sandboxed deployment.
