...
| --- | --- | --- | --- |
| `quickjs` | [quickjs-ng](https://github.com/quickjs-ng/quickjs) compiled to WebAssembly, run in-process on [wasmtime](https://wasmtime.dev) | FUSE-mounted workspace mounts; no host filesystem, no network | Yes |
...
mounts are visible when they are FUSE-mounted (the `fuse:` key on the
mount, or `add_fuse_mount`): the mountpoint is preopened into the run at
the mount's own path, so `std.open('/data/f.txt', 'r')` reads the mount
live and writes are immediately visible to every command (the I/O flows
through the kernel into the workspace, same cache and modes as `cat`).
Without a FUSE mount, mounts stay invisible: the `node`/`js` command still
resolves a script *file* through the workspace before running it, so
`node /data/script.mjs` works, but file I/O inside the code has nowhere to
land; use the `monty` Python runtime for mount I/O without FUSE.
...
option in the quickjs runtime's block (the `runtime_options` argument,
the yaml `runtime: quickjs:` block, or the MIRAGE_QUICKJS_HOME
...
ws = Workspace({"/data": RAMResource()},
               mode=MountMode.EXEC,
               js_runtime="quickjs",
               runtime_options={"quickjs": {"home": "/path/to/qjs-dir"}})
```
