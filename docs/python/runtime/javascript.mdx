...
write raw, and there is no `console.error` (write stderr with
...
ws = Workspace({"/data": RAMResource()},
               mode=MountMode.EXEC,
               runtimes=[QuickJsRuntime(config={"home": "/path/to/qjs-dir"}), "vfs"])
```
...
world has a [policy engine](/home/policy-engine): `policy: ./policy.js`
