...
Known gap: the Python `quickjs` runtime can expose FUSE-mounted workspace
mounts to the run (its engine runs on WASI preopens); quickjs-emscripten
has no equivalent, so TypeScript sandbox code cannot reach mounts yet.
