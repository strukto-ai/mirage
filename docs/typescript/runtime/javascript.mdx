...
reach your files or the network, but CPU and memory are bounded only by soft
engine caps and `command_safeguards` timeouts. For untrusted code or hard
resource limits, run behind a sandboxed deployment.
