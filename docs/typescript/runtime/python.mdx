...
Writes flush before the child runs, and the guest's cached files invalidate after
it completes. `Popen`, terminals and unsupported options fail explicitly. The
bridge is unavailable in the fallback without shared-memory workers.

Monty instead exposes `await mirage_run(argv, input, cwd)`, returning a mapping
with text `stdout`, `stderr` and `returncode`.
