...
description: Set up Supabase Storage as a MIRAGE VFS from Node or the browser.
---
...
```ts
import { MountMode, SupabaseVFS, Workspace } from '@struktoai/mirage-node'

const sb = new SupabaseVFS({
...
```ts
import { MountMode, SupabaseVFS, Workspace } from '@struktoai/mirage-browser'
...
See the [Supabase VFS](/python/vfs/supabase) docs for the equivalent Python wiring.
