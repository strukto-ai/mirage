...
title: S3
icon: aws
description: Set up AWS S3 as a MIRAGE VFS from Node or the browser.
---

MIRAGE ships `S3VFS` in **two runtimes**:
...
import { S3VFS } from '@struktoai/mirage-node'
...
The browser `S3VFS` never sees AWS credentials. You implement one function, `presignedUrlProvider`, that your frontend calls to get a signed URL for each S3 operation. The actual signing happens on **your server**, using any S3-signing SDK you prefer.
...
```ts
import { MountMode, S3VFS, Workspace } from '@struktoai/mirage-browser'
...
const ws = new Workspace({ '/bucket/': s3 }, { mode: MountMode.READ })
const res = await ws.shell('cat /bucket/hello.txt')
console.log(res.stdoutText)
```

### Customize a builtin command

Backend command definitions are available before a workspace is created. Copy
the definition with the behavior you want to change, then register that copy on
one mount:

```ts
import { S3_COMMANDS } from '@struktoai/mirage-browser'

const cat = S3_COMMANDS.require('cat')
const loggedCat = cat.withOverrides({
  fn: async (accessor, paths, texts, opts) => {
    console.log('reading', paths.map((path) => path.virtual))
    return cat.fn(accessor, paths, texts, opts)
  },
})

ws.mount('/bucket/')?.register(loggedCat)
```

Use the same `S3_COMMANDS` export from `@struktoai/mirage-node` for a Node
workspace. The builtin definition and other mounts stay unchanged.
Registration is a host application action; an agent can discover and run the
resulting `cat` command, but cannot alter the command table.
