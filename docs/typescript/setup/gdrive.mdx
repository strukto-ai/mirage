...
`GDriveVFS` exposes a user's Drive (My Drive plus shared drives the user can see) as a tree.
...
```ts
import { GDriveVFS, MountMode, Workspace } from '@struktoai/mirage-node'

const drive = new GDriveVFS({
...
For the mounted layout, the `gws` command reference, and snapshot behavior see the [Python Drive docs](/python/vfs/gdrive).
