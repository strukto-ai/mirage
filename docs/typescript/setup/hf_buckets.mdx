...
`HfBucketsVFS` mounts a [Hugging Face Bucket](https://huggingface.co/docs/buckets)
at a prefix such as `/hf/`. The TypeScript backend mirrors the
[Python one](/python/vfs/hf_buckets) and returns identical results.
...
```ts
import { HfBucketsVFS, MountMode, Workspace } from '@struktoai/mirage-node'

const bucket = new HfBucketsVFS({
...
console.log((await ws.shell('ls /hf/')).stdoutText)
console.log((await ws.shell('tree /hf/')).stdoutText)
console.log((await ws.shell("find /hf/ -name '*.json'")).stdoutText)
console.log((await ws.shell('stat /hf/data/file.txt')).stdoutText)
```

`HfBucketsVFS` takes the bucket in `namespace/bucket-name` form plus an
