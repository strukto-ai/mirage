...
of the macFUSE kernel extension. This works from TypeScript: `fuse.node`
links `/usr/local/lib/libfuse.2.dylib` by absolute path, so `backend:
'fskit'` reaches the same macFUSE 5.x libfuse the Python package uses.
