...
```ts
import { Mount, MountMode, RAMVFS, Workspace } from '@struktoai/mirage-node'

const ws = new Workspace({
  '/data': new Mount(new RAMVFS(), { mode: MountMode.WRITE, backend: MountBackend.FUSE }),
})
await ws.fuseReady()
...
Programmatically, set `backend` per mount: `{ '/data': new Mount(dataVfs, { backend: MountBackend.FUSE, mountpoint: '/tmp/data-repo' }), '/s3': new Mount(s3Vfs, { backend: MountBackend.FUSE }) }`.
...
```ts
const ws = new Workspace({
  '/data': new Mount(dataVfs, { backend: MountBackend.FSKIT }),
})
await ws.fuseReady()
...
system), and every mounted VFS should report exact sizes
(`sizesAlwaysKnown`; the `checkSizes` guard warns about anything else by
name, and those files read as empty).

**TS fskit mounts are read-mostly**, twice over. The shim's data-write
zeroing (see
[the Python page](/python/setup/fuse#mounting-without-the-kernel-extension-fskit))
applies to both languages, and `checkWrites` warns at mount time on
writable fskit mounts. On top of that, TypeScript cannot create new names:
`create`, `mkdir` and `rename` return ENOSYS (and a failed create can
still apply). The shim finalizes new items through macFUSE's Darwin-only
`setattr_x`/`renamex` callbacks, which Python installs at runtime
(`mirage/fuse/darwin.py`) but `@zkochan/fuse-native`'s compiled op table
cannot gain from JavaScript.
