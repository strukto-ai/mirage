...
icon: server
description: Set up Oracle Cloud Object Storage as a MIRAGE VFS from Node or the browser.
---
...
```ts
import { MountMode, OCIVFS, Workspace } from '@struktoai/mirage-node'

const oci = new OCIVFS({
...
The browser `OCIVFS` is secret-free, your backend signs each operation using your OCI customer secret keys and returns a URL. OCI's S3-compat endpoint accepts AWS Signature V4, so `@aws-sdk/s3-request-presigner` works, pointed at the OCI endpoint.
...
```ts
import { MountMode, OCIVFS, Workspace } from '@struktoai/mirage-browser'
...
See the [OCI VFS](/python/vfs/oci) docs for the equivalent Python wiring.
