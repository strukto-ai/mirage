...
icon: google
description: Set up Google Cloud Storage as a MIRAGE VFS from Node or the browser.
---
...
```ts
import { GCSVFS, MountMode, Workspace } from '@struktoai/mirage-node'

const gcs = new GCSVFS({
...
The browser `GCSVFS` is secret-free, your backend signs each operation using your HMAC keys and hands back a URL. Since GCS accepts AWS Signature V4, the same `@aws-sdk/s3-request-presigner` code used for S3 works, pointed at the GCS endpoint.
...
```ts
import { GCSVFS, MountMode, Workspace } from '@struktoai/mirage-browser'
...
See the [GCS VFS](/python/vfs/gcs) docs for the equivalent Python wiring.
