...
```ts
import { DatabricksVolumeApiError, DatabricksVolumeAuthError } from '@struktoai/mirage-core'
...
try {
  ...
  } else if (err instanceof DatabricksVolumeApiError) {
    // every other Files API refusal, with err.statusCode and err.errorCode
  }
}
```
...
a message on stderr rather than as a thrown error:

```ts
const result = await ws.execute('ls /dbx/')
