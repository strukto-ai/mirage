...
description: Set up a Databricks Unity Catalog volume as a MIRAGE VFS from Node.
---
...
const vfs = await DatabricksVolumeVFS.create(
  normalizeDatabricksVolumeConfig({
    catalog: 'main',
    schema: 'default',
    volume: 'agent_files',
...
const res = await ws.shell('ls /dbx/')
