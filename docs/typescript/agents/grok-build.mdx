...
[Grok Build](https://x.ai/cli) is xAI's coding-agent CLI; its executable is named `grok`. Mirage ships a Grok-compatible plugin with a guidance skill, one skill per virtualized CLI (`linear`, `slack`, `discord`, `gws`, `ntn`), and six filesystem tools: `read`, `write`, `edit`, `ls`, `grep`, and `execute_command`.

<Note>
  Grok Build can work on codebases in any programming language. The Mirage tool
  server itself requires Node.js 20.10 or newer.
</Note>
