...
```ts
import { Mount, MountMode, RAMVFS, S3VFS, Workspace } from '@struktoai/mirage-node'

const ws = new Workspace({
  '/': new Mount(new RAMVFS(), { mode: MountMode.WRITE, backend: MountBackend.FUSE }),
  '/s3/': new Mount(new S3VFS({ bucket: 'my-bucket', /* ... */ }), { mode: MountMode.READ }),
})
await ws.fuseReady()
