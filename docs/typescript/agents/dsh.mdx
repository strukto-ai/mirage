...
| Harness | Where Mirage sits | A session is |
| --- | --- | --- |
| DeepSeek Harness (this page) | its filesystem and shell | objects in your process — no subprocess, roughly 0.5 ms and 240 KB each |
| [Codex](/typescript/agents/codex) | tools beside the CLI's native ones | a Codex CLI process plus a Node tool server |
| [Claude Agent SDK](/typescript/agents/claude-agent-sdk) | MCP tools served from your process | a spawned Claude Code CLI process |
