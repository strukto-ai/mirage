...
Everything above is purely in memory — the workspace, both providers, and monty are objects in the host process, with no subprocess, no disk, and no kernel mount — so a world can be created per request and garbage-collected with it.
