...
title: Dropbox
icon: dropbox
description: Mount Dropbox as a read/write MIRAGE VFS from Node or the browser via OAuth2.
---

Mirage ships `DropboxVFS` in **two runtimes**:
...
backend, see [Dropbox (Python)](/python/vfs/dropbox).
...
```ts
import { DropboxVFS, MountMode, Workspace } from '@struktoai/mirage-node'
...
const res = await ws.shell('ls /dropbox/')
...
const ws = new Workspace({ '/dropbox': dropbox }, { mode: MountMode.READ })
await ws.shell('ls /dropbox/') // lists the contents of /Team/data
```

`rootPath` accepts `Team/data`, `/Team/data`, or `/Team/data/` (all normalized the same way);
`..` segments are rejected. Config dictionaries (e.g. via the VFS registry) may spell it
...
[Dropbox (Python) — Search push-down](/python/vfs/dropbox#search-push-down) for the
...
import { DropboxVFS, MountMode, patchNodeFs, Workspace } from '@struktoai/mirage-node'
...
```ts
import { DropboxVFS, Mount, MountMode, Workspace } from '@struktoai/mirage-node'
...
`DropboxVFS` ships the same shell command set as the GDrive VFS:
...
- [`dropbox.ts`](https://github.com/strukto-ai/mirage/blob/main/examples/typescript/dropbox/dropbox.ts), `Workspace.shell('ls/stat/cat/tree …')` shell demo
...
`grep` classifies the bytes it reads, including downloaded attachments. Use `-I` to skip binary inputs or `-a` (`--binary-files=text`) to search them as text. Text mode bypasses the content-search index and scans files directly.
