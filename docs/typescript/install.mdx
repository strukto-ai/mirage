...
This ships the base runtime (Workspace, RAM VFS, Disk VFS, the shell executor) and is enough to follow the [TypeScript Quickstart](/typescript/quickstart).
...
<Tabs>
  ...
  <Tab title="Redis">
    Required for `RedisVFS`, `RedisFileCacheStore`, and `RedisIndexCacheStore`.
  ...
  <Tab title="MongoDB">
    Required for `MongoDBVFS` and `GridFSVFS`.
  ...
  <Tab title="Email">
    Required for `EmailVFS` (IMAP read, SMTP send, MIME parsing).
...
const res = await ws.shell('echo "hello" | tee /data/x.txt')
