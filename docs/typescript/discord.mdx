...
MIRAGE ships `DiscordVFS` in **two runtimes**:
...
Both runtimes expose the same filesystem shape (`/discord/<guild>/channels/<channel>/<date>/{chat.jsonl,files/}`, `/discord/<guild>/members/`) and the same shell commands, and both pair with the [discord CLI](/typescript/cli/discord) for acting on the server.
...
```ts
import { DiscordVFS, MountMode, Workspace } from '@struktoai/mirage-node'

const discord = new DiscordVFS({
  token: process.env.DISCORD_BOT_TOKEN!,
})

const ws = new Workspace({ '/discord': discord }, { mode: MountMode.READ })
const res = await ws.shell('ls /discord/')
...
[discord CLI](/typescript/cli/discord) when installed; the mounted
tree stays read-oriented.
...
- [`examples/typescript/discord/discord_browser/`](https://github.com/strukto-ai/mirage/tree/main/examples/typescript/discord/discord_browser), proxy server + `@struktoai/mirage-browser` DiscordVFS demo.

See [Python Discord VFS](/python/vfs/discord) for the equivalent Python wiring.
