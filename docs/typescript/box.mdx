...
description: Mount Box as a MIRAGE VFS from Node or the browser via OAuth2.
---

Mirage ships `BoxVFS` in **two runtimes**:
...
const ws = new Workspace({ '/box': box }, { mode: MountMode.READ })
const res = await ws.shell('ls /box/')
...
import { BoxVFS, MountMode, patchNodeFs, Workspace } from '@struktoai/mirage-node'
...
```ts
import { BoxVFS, Mount, MountMode, Workspace } from '@struktoai/mirage-node'
...
mapping in the `RAMIndexCacheStore` attached to the VFS. The first time you `ls /box/foo/bar/`,
...
The cache TTL defaults to 24h. To force re-resolution, recreate the `BoxVFS`.
...
`BoxVFS` ships the same shell command set as `GDriveVFS` and `DropboxVFS`:
...
- [`box.ts`](https://github.com/strukto-ai/mirage/blob/main/examples/typescript/box/box.ts), `Workspace.shell('ls/stat/cat/tree …')` shell demo
