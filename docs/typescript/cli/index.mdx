...
Once a CLI
is installed, the workspace's system prompt lists it by that skill's
frontmatter `description`, and `man <cli>` (no verb) leads with the
skill's body before the usual verb listing. The same
`plugins/mirage/skills/` directory is what the Codex, Grok Build and
Claude Code plugins load, so a skill is written once and read from five
places.
