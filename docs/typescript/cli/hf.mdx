...
const ws = new Workspace({
  '/m': new HfModelsVFS({ repoId: 'acme/model', token }),
  '/work': new Mount(new RAMVFS(), { mode: MountMode.WRITE }),
})
...
The Hub VFS need no extra dependency: they speak the Hub API over the
runtime's own `fetch`. `HfBucketsVFS` is a different product, the Xet
