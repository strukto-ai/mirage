...
from mirage.core.gmail.stat import stat as gmail_stat
...
async def _resolve_sizes(
    ...
    for p in paths:
        ...
        if size is None:
            try:
                file_stat = await gmail_stat(accessor, p, index)
                size = file_stat.size
...
async def file_read_provision(
    ...
    **_extra: object,
    ...
    """Cost estimate for full file reads (cat, wc) over Google Mail."""
    if not paths:
...
async def metadata_provision(command: str = "",
    ...
    """Cost estimate for metadata-only ops over Google Mail."""
    return ProvisionResult(
        command=command,
        network_read_low=0,
        network_read_high=0,
