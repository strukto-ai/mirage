...
async def run_gws_method(
    ...
    **_extra: object,
    ...
    if method.raw_bytes:
        sep = "&" if "?" in url else "?"
        pairs = "&".join(f"{k}={v}" for k, v in query_params.items())
        data = await google_get_bytes(token_manager,
                                      url + (sep + pairs if pairs else ""))
        return yield_bytes(data), IOResult()
    if method.http == "GET":
        result = await google_get(token_manager, url, params=query_params)
    elif method.http == "POST":
        result = await google_post(token_manager,
                                   _with_query(url, query_params), body)
        await invalidate_mount_listing()
    elif method.http == "PATCH":
        result = await google_patch(token_manager,
                                    url,
                                    body,
                                    params=query_params)
        await invalidate_mount_listing()
    elif method.http == "DELETE":
        await google_delete(token_manager, _with_query(url, query_params))
        await invalidate_mount_listing()
        return None, IOResult()
    else:
        raise ValueError(f"unsupported method {method.http}")
