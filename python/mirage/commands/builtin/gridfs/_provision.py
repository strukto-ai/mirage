...
async def _resolve_sizes(
    ...
    Order of resolution per path:
        ...
        2. fs.files lookup via ``gridfs_stat`` -- one metadata query.
           Populates the index as a side effect, so the next provision
           is free.
    ...
    for p in paths:
        ...
        if size is None:
            try:
                file_stat = await gridfs_stat(accessor, p, index)
                size = file_stat.size
