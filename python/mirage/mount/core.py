...
class MountCore:
    ...
    async def prefetch_read(self, path: str) -> bytes | None:
        ...
        try:
            ...
            logger.debug(
                "mount: hydration read of %s failed, deferring to read(): %r",
                path, err)
            return None
        ...
        self._prefetch.put(self.identity(path), data)
        return data
    ...
    async def open(self, path: str, flags: int = 0) -> int:
        ...
        if flags & os.O_TRUNC:
            ...
            # NFSv3 has no OPEN, so this rung is reached only from FUSE.
    ...
    async def truncate(self, path: str, length: int) -> None:
        ...
        for ctx in self._handles.values():
            if ctx.key == key and ctx.write_buf:
                await self._apply_writes(ctx.path, ctx.write_buf)
                ctx.write_buf = []
    ...
    async def _changed(self, path: str, rehydrate: bool = True) -> None:
        ...
        try:
            ...
            logger.warning("mount: refresh of %s after a change failed: %r",
                           path, err)
            for ctx in hydrated:
                ctx.data = None
            return
