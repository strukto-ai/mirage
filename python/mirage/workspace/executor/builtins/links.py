...
def handle_ln(
    ...
    exists = link_abs in registry.symlinks and "f" not in flags
    if registry.is_mount_root(link_abs) or exists:
        ...
        return None, IOResult(exit_code=1,
                              stderr=err), ExecutionNode(command="ln",
                                                         exit_code=1,
                                                         stderr=err)
    registry.symlinks[link_abs] = target_typed
    out = None
    if "v" in flags:
...
def handle_readlink(
    ...
    exit_code = 0
    for op in operands:
        target = registry.symlinks.get(_abs(op, session.cwd))
        if target is None:
            exit_code = 1
            continue
        lines.append(target)
