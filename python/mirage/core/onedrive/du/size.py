...
async def size(accessor: OneDriveAccessor,
               path: PathSpec,
               index: IndexCacheStore = NULL_INDEX) -> int:
    """Recursive byte size of everything under a path.

    A file has no ``/children`` endpoint (Graph 404s on it), so the file
    case resolves from its own stat and only a directory is walked.
