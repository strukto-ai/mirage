...
async def build_entity_schema_json(accessor: PostgresAccessor, schema: str,
    ...
    async with pool.acquire() as conn:
        ...
        idx = await client.fetch_indexes(conn, schema, name)
        rows = await client.estimated_row_count(conn, schema, name)
        size = await client.table_size_bytes(conn, schema, name)
    pk_set = set(pk)
