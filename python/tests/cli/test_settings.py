...
def test_load_daemon_settings_falls_back_to_token_file(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "mirage.server.auth.storage.DEFAULT_TOKEN_FILE", token_file)
...
def test_load_daemon_settings_no_sources_yields_empty_token(tmp_path,
                                                            monkeypatch):
    monkeypatch.delenv("MIRAGE_TOKEN", raising=False)
    monkeypatch.delenv("MIRAGE_DAEMON_URL", raising=False)
    config_file = tmp_path / "config.toml"
    config_file.write_text("")
    monkeypatch.setattr(
        "mirage.server.auth.storage.DEFAULT_TOKEN_FILE",
        tmp_path / "missing")
    settings = load_daemon_settings(path=config_file)
