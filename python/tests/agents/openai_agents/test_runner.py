...
def ws():
    workspace = MagicMock()
    workspace.vfs = MagicMock()
    workspace.vfs.stat = AsyncMock()
    workspace.vfs.read = AsyncMock()
    return workspace


@pytest.mark.asyncio
async def test_build_blocks_image_inlines_base64(ws):
    ws.vfs.stat.return_value = _stat(ContentType.IMAGE_PNG)
    ws.vfs.read.return_value = b"\x89PNG\r\n\x1a\n"
...
async def test_build_blocks_pdf_uploads_to_files_api(ws):
    ws.vfs.stat.return_value = _stat(ContentType.PDF)
    ws.vfs.read.return_value = b"%PDF-1.4 ..."
...
async def test_build_blocks_text_decoded_inline(ws):
    ws.vfs.stat.return_value = _stat(ContentType.TEXT)
    ws.vfs.read.return_value = b"hello world"
    runner = MirageRunner(ws)
...
async def test_build_blocks_jpeg(ws):
    ws.vfs.stat.return_value = _stat(ContentType.IMAGE_JPEG)
    ws.vfs.read.return_value = b"\xff\xd8\xff..."
    runner = MirageRunner(ws)
...
async def test_build_blocks_multiple_paths_in_order(ws):
    ...
    ws.vfs.stat.side_effect = fake_stat
    ws.vfs.read.side_effect = fake_read
    runner = MirageRunner(ws)
