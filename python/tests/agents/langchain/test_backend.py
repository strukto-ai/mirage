...
async def test_the_host_door_still_sees_everything(workspace):
    ...
    backend = LangchainWorkspace(workspace)
    read = await backend.aread("/key.pem")
    assert read.file_data is not None
    assert "PRIVATE" in read.file_data["content"]
