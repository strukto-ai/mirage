...
async def test_pipe_grep_sort_uniq(ws):
    io = await ws.shell("cat /data/numbers.txt | sort | uniq")
...
async def test_pipe_grep_wc(ws):
    io = await ws.shell("grep ERROR /data/log.txt | wc -l")
    assert (await io.stdout_str()).strip() == "2"


@pytest.mark.asyncio
async def test_pipe_head_stops_early(ws):
    io = await ws.shell("cat /data/big.txt | head -n 3")
    lines = (await io.stdout_str()).strip().split("\n")
    assert len(lines) == 3
...
async def test_pipe_tail(ws):
    io = await ws.shell("cat /data/log.txt | tail -n 2")
    lines = (await io.stdout_str()).strip().split("\n")
    assert len(lines) == 2
...
async def test_triple_pipe(ws):
    io = await ws.shell("cat /data/log.txt | grep INFO | head -n 2")
    lines = (await io.stdout_str()).strip().split("\n")
    assert len(lines) == 2
...
async def test_and_success(ws):
    io = await ws.shell("cat /data/hello.txt && echo done")
...
async def test_or_fallback(ws):
    io = await ws.shell("grep NONEXISTENT /data/hello.txt || echo fallback")
    assert b"fallback" in io.stdout


@pytest.mark.asyncio
async def test_semicolon_runs_both(ws):
    io = await ws.shell("echo first ; echo second")
...
async def test_redirect_stdout_to_file(ws):
    ...
    io = await ws.shell("cat /data/out.txt")
...
async def test_redirect_append(ws):
    ...
    io = await ws.shell("cat /data/append.txt")
...
async def test_redirect_stdin(ws):
    io = await ws.shell("grep world < /data/hello.txt")
    assert b"world" in io.stdout


@pytest.mark.asyncio
async def test_heredoc(ws):
    io = await ws.shell("cat << EOF\nhello heredoc\nEOF")
...
async def test_subshell_inherits_parent_env(ws):
    ...
    io = await ws.shell("(printenv INHERITED)")
...
async def test_export_then_variable_expansion(ws):
    ...
    io = await ws.shell("grep $PATTERN /data/log.txt | wc -l")
    assert (await io.stdout_str()).strip() == "2"
...
async def test_two_sessions_isolated_cwd(ws):
    ...
    sb.cwd = "/"
    await ws.shell("cd /data", session_id="s-a")
    await ws.shell("cd /data/subdir", session_id="s-b")
    assert sa.cwd == "/data"
...
async def test_two_sessions_isolated_env(ws):
    ...
    await ws.shell("export X=from_a", session_id="s-a")
    await ws.shell("export X=from_b", session_id="s-b")
...
async def test_session_env_not_visible_cross_session(ws):
    ws.create_session("s-a")
    ws.create_session("s-b")
    await ws.shell("export PRIVATE=yes", session_id="s-a")
    io = await ws.shell("printenv PRIVATE", session_id="s-b")
    assert io.exit_code == 1
...
async def test_cd_export_grep_pipe(ws):
    ...
    io = await ws.shell("grep $TERM log.txt | wc -l")
    assert (await io.stdout_str()).strip() == "2"
...
async def test_subshell_with_redirect(ws):
    ...
    io = await ws.shell("cat /data/sub_out.txt")
...
async def test_pipe_into_redirect(ws):
    ...
    io = await ws.shell("cat /data/errors.txt")
    lines = (await io.stdout_str()).strip().split("\n")
    assert len(lines) == 2
...
async def test_history_tracks_session_id(ws):
    ...
    await ws.shell("echo tracked", session_id="s-hist")
...
async def test_grep_no_match_exit_code(ws):
    io = await ws.shell("grep NONEXISTENT /data/hello.txt")
    assert io.exit_code == 1


@pytest.mark.asyncio
async def test_grep_match_exit_code(ws):
    io = await ws.shell("grep hello /data/hello.txt")
    assert io.exit_code == 0


@pytest.mark.asyncio
async def test_grep_q_match(ws):
    io = await ws.shell("grep -q world /data/hello.txt")
    assert io.exit_code == 0
...
async def test_grep_q_no_match(ws):
    io = await ws.shell("grep -q NONEXISTENT /data/hello.txt")
    assert io.exit_code == 1
...
async def test_grep_pipe_no_match_last_stage_wins(ws):
    io = await ws.shell("grep NONEXISTENT /data/hello.txt | sort")
    assert io.exit_code == 0


@pytest.mark.asyncio
async def test_grep_pipe_match(ws):
    io = await ws.shell("grep ERROR /data/log.txt | sort")
    assert io.exit_code == 0
    lines = (await io.stdout_str()).strip().split("\n")
    assert len(lines) == 2
    assert lines[0] == "ERROR bad"
...
async def test_grep_count_no_match(ws):
    io = await ws.shell("grep -c NONEXISTENT /data/hello.txt")
    assert (await io.stdout_str()).strip() == "0"


@pytest.mark.asyncio
async def test_grep_count_match(ws):
    io = await ws.shell("grep -c ERROR /data/log.txt")
    assert io.exit_code == 0
    assert (await io.stdout_str()).strip() == "2"
...
async def test_grep_invert_match(ws):
    io = await ws.shell("grep -v ERROR /data/log.txt")
    assert io.exit_code == 0
    lines = (await io.stdout_str()).strip().split("\n")
...
async def test_grep_invert_no_output(ws):
    io = await ws.shell("echo hello | grep -v hello")
    assert not (await io.stdout_str()).strip()
...
async def test_rg_no_match_exit_code(ws):
    io = await ws.shell("rg NONEXISTENT /data/hello.txt")
    assert io.exit_code == 1


@pytest.mark.asyncio
async def test_rg_match_exit_code(ws):
    io = await ws.shell("rg hello /data/hello.txt")
    assert io.exit_code == 0


@pytest.mark.asyncio
async def test_rg_no_match_and_chain(ws):
    io = await ws.shell("rg NONEXISTENT /data/hello.txt && echo found")
    assert b"found" not in (io.stdout or b"")


@pytest.mark.asyncio
async def test_rg_no_match_or_chain(ws):
    io = await ws.shell("rg NONEXISTENT /data/hello.txt || echo fallback")
    assert b"fallback" in io.stdout


@pytest.mark.asyncio
async def test_rg_pipe_no_match(ws):
    io = await ws.shell("rg NONEXISTENT /data/hello.txt | wc -l")
    assert io.exit_code == 0


@pytest.mark.asyncio
async def test_rg_match_pipe_head(ws):
    io = await ws.shell("rg INFO /data/log.txt | head -n 1")
    assert io.exit_code == 0
...
async def test_diff_identical_files(ws):
    ...
    io = await ws.shell("diff /data/diff_a.txt /data/diff_b.txt")
    assert io.exit_code == 0
    assert not (await io.stdout_str()).strip()
...
async def test_find_existing(ws):
    io = await ws.shell("find /data/subdir")
    assert io.exit_code == 0
...
async def test_find_with_name(ws):
    io = await ws.shell("find /data/subdir -name a.txt")
    assert io.exit_code == 0
    assert b"a.txt" in io.stdout
...
async def test_grep_match_and_diff(ws):
    ...
    io = await ws.shell("diff /data/errors.txt /data/expected.txt")
    assert io.exit_code in (0, 1)


@pytest.mark.asyncio
async def test_rg_subshell_isolation(ws):
    io = await ws.shell("(rg NONEXISTENT /data/hello.txt) || echo recovered")
