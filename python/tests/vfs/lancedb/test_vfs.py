...
def _vfs(**kw) -> LanceDBVFS:
    base = dict(uri="/tmp/db", group_by=["label"], id_column="id")
    base.update(kw)
    return LanceDBVFS(LanceDBConfig(**base))


def test_vfs_name_local_is_not_remote():
    res = _vfs()
    assert res.name == "lancedb"
    assert res.caches_reads is False


def test_vfs_remote_uri_caches_reads():
    res = _vfs(uri="s3://bucket/db")
...
async def test_vfs_in_registry():
    ...
    res = build_vfs("lancedb", {"uri": "/tmp/db"})
    assert res.name == "lancedb"
