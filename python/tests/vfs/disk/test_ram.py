...
def test_mkdir_and_readdir(memory_backend):
    asyncio.run(ops(memory_backend).mkdir(_ps("/mydir")))
    entries = asyncio.run(ops(memory_backend).readdir(_ps("/mydir")))
    assert entries == []


def test_mkdir_missing_parent_raises(memory_backend):
    with pytest.raises(FileNotFoundError):
        asyncio.run(ops(memory_backend).mkdir(_ps("/a/b/c")))


def test_rmdir_empty(memory_backend):
    asyncio.run(ops(memory_backend).mkdir(_ps("/emptydir")))
    asyncio.run(ops(memory_backend).rmdir(_ps("/emptydir")))
    with pytest.raises(FileNotFoundError):
        asyncio.run(ops(memory_backend).stat(_ps("/emptydir")))


def test_rmdir_nonempty_raises(memory_backend):
    asyncio.run(ops(memory_backend).mkdir(_ps("/dir")))
    asyncio.run(ops(memory_backend).write(_ps("/dir/file.txt"), b""))
    with pytest.raises(OSError):
        asyncio.run(ops(memory_backend).rmdir(_ps("/dir")))


def test_unlink(memory_backend):
    asyncio.run(ops(memory_backend).write(_ps("/del.txt"), b""))
    asyncio.run(ops(memory_backend).unlink(_ps("/del.txt")))
    with pytest.raises(FileNotFoundError):
...
def test_unlink_missing_raises(memory_backend):
    with pytest.raises(FileNotFoundError):
        asyncio.run(ops(memory_backend).unlink(_ps("/nope.txt")))


def test_rename_file(memory_backend):
    asyncio.run(ops(memory_backend).write(_ps("/old.txt"), b"content"))
    asyncio.run(ops(memory_backend).rename(_ps("/old.txt"), _ps("/new.txt")))
...
def test_stat_file(memory_backend):
    asyncio.run(ops(memory_backend).write(_ps("/f.txt"), b"hello"))
    s = asyncio.run(ops(memory_backend).stat(_ps("/f.txt")))
    assert s.name == "f.txt"
    assert s.size == 5
    assert s.type == "file"
...
def test_stat_directory(memory_backend):
    asyncio.run(ops(memory_backend).mkdir(_ps("/mydir")))
    s = asyncio.run(ops(memory_backend).stat(_ps("/mydir")))
...
def test_stat_missing_raises(memory_backend):
    with pytest.raises(FileNotFoundError):
        asyncio.run(ops(memory_backend).stat(_ps("/missing.txt")))


def test_exists_true(memory_backend):
    asyncio.run(ops(memory_backend).write(_ps("/f.txt"), b""))
    try:
        asyncio.run(ops(memory_backend).stat(_ps("/f.txt")))
        exists = True
...
def test_exists_false(memory_backend):
    try:
        asyncio.run(ops(memory_backend).stat(_ps("/nope.txt")))
        exists = True
    except FileNotFoundError:
        exists = False
    assert not exists
...
def test_readdir_lists_direct_children(memory_backend):
    asyncio.run(ops(memory_backend).mkdir(_ps("/parent")))
    asyncio.run(ops(memory_backend).mkdir(_ps("/parent/child1")))
    asyncio.run(ops(memory_backend).write(_ps("/parent/file.txt"), b""))
    entries = asyncio.run(ops(memory_backend).readdir(_ps("/parent")))
