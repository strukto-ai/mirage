...
# lived in core/github/_client.py because GitHubVFS.__init__ had to
# fetch the repo tree and a constructor cannot await. The fetch moved to
# the async `BaseVFS.build` factory, so nothing needs it any more.
...
def test_no_blocking_http_client_in_the_package() -> None:
    # A blocking call inside a VFS constructor freezes whatever event
