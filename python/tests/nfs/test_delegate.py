...
class FakeLinks:
    ...
    def follow(self, path: str) -> str:
        """Resolve a link prefix in ``path``, identity when none matches.

        The core keys every cache it holds on this, so answering a bare
        identity would hide the aliasing it exists to recognise. One hop
        is enough for a fake: no test here builds a chain, and the real
        view is the authority on the hop limit. A relative target is
        resolved against the link's own directory, the way the
        namespace resolves one.

        Args:
            path (str): absolute virtual path.
        """
        for link, target in self._table.items():
            if path != link and not path.startswith(link + "/"):
                continue
            base = (target if target.startswith("/") else posixpath.normpath(
                posixpath.join(posixpath.dirname(link), target)))
            return base + path[len(link):]
        return path
