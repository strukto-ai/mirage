...
async def test_stat_op(tmp_path):
    ...
    scope = PathSpec(vfs_path="f.txt", virtual="/f.txt", directory="/")
...
async def test_readdir_op(tmp_path):
    ...
    accessor = DiskAccessor(tmp_path)
    index = RAMIndexCacheStore(ttl=0)
    scope = PathSpec(vfs_path="", virtual="/", directory="/", resolved=False)
...
async def test_write_op(tmp_path):
    accessor = DiskAccessor(tmp_path)
    scope = PathSpec(vfs_path="out.txt", virtual="/out.txt", directory="/")
