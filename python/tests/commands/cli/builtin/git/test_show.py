...
async def test_no_operand_shows_head(git_ws):
    result = await git_ws.shell("git -C /repo show")
...
async def test_the_header_block_comes_before_the_patch(git_ws):
    result = await git_ws.shell("git -C /repo show HEAD")
...
async def test_an_ancestry_suffix_walks_back(git_ws):
    result = await git_ws.shell("git -C /repo show HEAD~2")
...
async def test_a_commit_id_operand_resolves(git_ws):
    listing = await git_ws.shell("git -C /repo log --oneline -n 1")
    short = listing.stdout.decode().split(" ", 1)[0]
    result = await git_ws.shell(f"git -C /repo show {short}")
    assert result.exit_code == 0
    assert "    third" in result.stdout.decode()
...
async def test_an_unknown_revision_is_a_fatal(git_ws):
    result = await git_ws.shell("git -C /repo show nope")
...
async def test_walking_off_the_end_of_history_is_a_fatal(git_ws):
    result = await git_ws.shell("git -C /repo show HEAD~9")
    assert result.exit_code == 128
...
async def test_a_merge_names_its_parents(repo_path):
    ...
    with mounted(repo_path) as ws:
        ...
        result = await ws.shell("git -C /repo show HEAD")
    lines = result.stdout.decode().split("\n")
    assert lines[0].startswith("commit ")
...
async def test_name_only_lists_bare_paths(git_ws):
    result = await git_ws.shell("git -C /repo show --name-only HEAD")
    text = result.stdout.decode()
...
async def test_name_only_wins_over_stat_like_git(git_ws):
    result = await git_ws.shell("git -C /repo show --stat --name-only HEAD")
    text = result.stdout.decode()
    assert text.endswith("\n\na.txt\n")
...
async def test_no_patch_suppresses_every_diff_section(git_ws):
    plain = await git_ws.shell("git -C /repo show --no-patch HEAD")
    stat = await git_ws.shell("git -C /repo show --stat --no-patch HEAD")
...
async def test_s_is_the_short_spelling_of_no_patch(git_ws):
    dashed = await git_ws.shell("git -C /repo show -s HEAD")
    spelled = await git_ws.shell("git -C /repo show --no-patch HEAD")
    assert dashed.stdout == spelled.stdout


@pytest.mark.asyncio
async def test_s_with_format_answers_one_field(git_ws):
    result = await git_ws.shell("git -C /repo show -s --format=%s HEAD")
...
async def test_format_header_rides_over_the_patch(git_ws):
    result = await git_ws.shell("git -C /repo show --format=%s HEAD")
    text = result.stdout.decode()
...
async def test_no_ext_diff_is_accepted_and_changes_nothing(git_ws):
    flagged = await git_ws.shell("git -C /repo show --no-ext-diff HEAD")
    plain = await git_ws.shell("git -C /repo show HEAD")
...
async def test_show_format_header_prints_without_trailing_newline(git_ws):
    ...
    result = await git_ws.shell("git -C /repo show -s --pretty='format:%h'")
...
async def test_show_tformat_terminates_an_empty_expansion(git_ws):
    ...
    result = await git_ws.shell("git -C /repo show -s --format='%b'")
    assert result.stdout == b"\n"
    silent = await git_ws.shell("git -C /repo show -s --format=")
    assert not silent.stdout


@pytest.mark.asyncio
async def test_show_bare_format_is_refused_like_git(git_ws):
    result = await git_ws.shell("git -C /repo show --format")
    assert result.exit_code == 128
