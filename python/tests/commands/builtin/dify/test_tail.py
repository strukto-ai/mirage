...
@pytest.mark.asyncio
async def test_tail_reads_last_lines_from_stream(monkeypatch, dify_accessor,
    ...
    stdout, _ = await tail(dify_accessor, [guide_path], index=dify_index, n=1)

    assert await materialize(stdout) == b"gamma"


@pytest.mark.asyncio
async def test_tail_uses_read_stream(monkeypatch, dify_accessor, dify_index,
                                     guide_path):
    monkeypatch.setattr(tree, "list_all_documents", list_single_document)
    ...
    stdout, _ = await tail(dify_accessor, [guide_path], index=dify_index, n=2)

    assert await materialize(stdout) == b"beta\ngamma"


@pytest.mark.asyncio
async def test_tail_supports_byte_counts(monkeypatch, dify_accessor,
                                         dify_index, guide_path):
    monkeypatch.setattr(tree, "list_all_documents", list_single_document)
    monkeypatch.setattr(read, "iter_segment_pages", iter_bytes_pages)

    stdout, _ = await tail(dify_accessor, [guide_path],
                           c="2",
                           index=dify_index)

    assert await materialize(stdout) == b"ef"
