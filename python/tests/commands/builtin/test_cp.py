...
def test_cp_recursive(backend):
    ...
    asyncio.run(write_bytes(accessor, PathSpec.from_str_path("/tmp/src/a.txt"), b"aaa"))
    asyncio.run(write_bytes(accessor, PathSpec.from_str_path("/tmp/src/sub/b.txt"), b"bbb"))
