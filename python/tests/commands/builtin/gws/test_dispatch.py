...
async def test_dispatch_routes_api_method(accessor, monkeypatch):
    ...
    assert (method.service, method.resource, method.method) == \
        ("docs", "documents", "get")
    assert fake.await_args.kwargs["params"] == '{"documentId": "d1"}'


@pytest.mark.asyncio
async def test_dispatch_routes_bespoke_create(accessor, monkeypatch):
    fake = AsyncMock(return_value=(None, IOResult()))
    monkeypatch.setitem(dispatch_mod._BESPOKE, ("docs", "documents", "create"),
                        fake)
    await gws(accessor, [],
              "docs",
              "documents",
              ...
              json='{"title": "T"}')
    fake.assert_awaited_once()
    assert fake.await_args.kwargs["json"] == '{"title": "T"}'


@pytest.mark.asyncio
async def test_dispatch_routes_plus_helper_with_aliases(accessor, monkeypatch):
    fake = AsyncMock(return_value=(None, IOResult()))
    monkeypatch.setitem(dispatch_mod._PLUS, ("sheets", "+read"), fake)
