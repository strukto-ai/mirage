...
async def test_readdir_workspaces(accessor, index):
    ...
    with patch("mirage.core.trello.readdir.list_workspaces",
        ...
        result = await readdir(accessor, PathSpec.from_str_path("/workspaces"), index)
...
async def test_readdir_workspace_entry(accessor, index):
    ...
    result = await readdir(accessor, PathSpec.from_str_path("/workspaces/Engineering__ws1"), index)
    assert result == [
...
async def test_readdir_boards(accessor, index):
    ...
    with patch("mirage.core.trello.readdir.list_workspace_boards",
               new_callable=AsyncMock,
               return_value=boards):
        result = await readdir(accessor, PathSpec.from_str_path("/workspaces/Engineering__ws1/boards"),
                               index)
    assert result == [
