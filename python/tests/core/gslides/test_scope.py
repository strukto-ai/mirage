...
def test_file_splits_label_and_id():
    match = detect_scope(_ps("/owned/2024-01-05_Notes__abc12.gslide.json"))
...
def test_shared_file_matches_too():
    match = detect_scope(_ps("/shared/Plan__xyz.gslide.json"))
    assert match.kind == "file"
...
def test_invalid_shapes():
    ...
    assert detect_scope(_ps("/bogus/File__id.gslide.json")).kind == "invalid"
    assert detect_scope(_ps("/owned/plain.gslide.json")).kind == "invalid"
    assert detect_scope(_ps("/owned/File__id.wrong.json")).kind == "invalid"
    assert detect_scope(
        _ps("/owned/File__id.gslide.json/deep")).kind == "invalid"
